...
public class Calculation {
	...
	/**附法内丹伤害的计算方法*/
	public int petHurt(ManData mydata,ManData nomyadata,double jichu,double wg,String type,int death){
		double hs = mydata.getsx(1,type);
		double kx = nomyadata.getsx(0,type);
		double qf = 1+(mydata.getsx(2,type)/100D);
		double fl =296.1572+0.0002364957*Math.pow(mydata.getMp_z(),1.57);
		DecimalFormat decimalFormat = new DecimalFormat("#.####");
		String formattedNumber = decimalFormat.format(fl);

//		int hurt=(int) ((jichu+mydata.getsx(3,type))*(1+(hs-kx)/100)*qf*fl);
		int hurt=(int) ((1+(hs-kx)/100)*fl);
		if (hurt>2139999999) {hurt=1;WriteOut.addtxt("伤害上限:"+mydata.getType()+":"+mydata.getManname()+":"+mydata.getId(), 9999);}
		return hurt>1?hurt:1;
//		return (int)(296.1572+0.0002364957*Math.pow(mydata.getMp_z(),1.57));
	}
...
//	鬼三尸伤害
    ...
    public int getzdup(ManData mydata,FightingSkill skill,double qds,ManData nomyadata){
    	...
    	double szd = Integer.parseInt(configure.getDshsz());
...
//		}
		return (int) (nomyadata.getHp_z()*szd/100);
...
//		this.skillgain=(lvl+sld/100)*8+(skilllvl==5?10000:skilllvl==4?12000:8000);
    ...
    public int getzdsh(ManData mydata,ManData nomyadata,FightingSkill skill,double qds){
		...
		return (int) ((qds/100+1)*(skill.getSkillhurt())*five);
    }
}
