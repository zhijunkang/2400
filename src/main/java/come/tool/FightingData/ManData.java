...
public class ManData implements Cloneable {
	...
	/**召唤兽技能加载*/
	public void addPetSkill(String petSkills,long qm,FightingSummon summon,RoleSummoning pet){
			...
			if (skill!=null) {
				...
				if (skilltype==1) {
					...
					for (int j = 0; j < this.skills.size(); j++) {
						if (this.skills.get(j).getSkillid() == 1831|| this.skills.get(j).getSkillid() == 1833)
							this.skills.get(j).setSkillhurt(this.skills.get(j).getSkillhurt() + 15);
					}
					continue;
				}else {
					...
					FightingSkill fightingSkill=new FightingSkill(skill, this.lvl, this.zs, 350 + sld,qm,0);
					...
					if (skillid==1833||skillid==1831){   //分花拂柳  高级分裂 力量小于450不生效
						if (pet.getPower()<450)
							continue;
					}
					if (skillid==1274){
						...
						if (pet.getSummoningskin().equals("728")||pet.getSummoningskin().equals("729")){
	...
	public void addPetLX(String lingxi,long qm){
		...
		for (Skill skill : skills) {
			if (skill!=null) {
				FightingSkill fightingSkill=new FightingSkill(skill, this.lvl, this.zs, 250,qm,0);
	...
	public ManData(RoleSummoning pet,FightingSummon summon) {
		...
		addPetSkill(pet.getPetSkills(),qm,summon,pet);//召唤兽技能
		addPetSkill(pet.getPetQlSkills(),qm,summon,pet);//启灵技能
		addPetSkill(pet.getBeastSkills(),qm,summon,pet);//神兽技能
		addPetSkill(pet.getSkill(),qm,summon,pet);//天生技能
	...
	public ManData(Baby baby, int camp, int man,Map<String,Double> map) {
		...
		if (eid!=null){
			Goodstable goodsByRgID = AllServiceUtil.getGoodsTableService().getGoodsByRgID(BigDecimal.valueOf(Long.parseLong(eid)));
			if(goodsByRgID!=null){
				String s = goodsByRgID.getGoodsid().toString();
				if ("647".equals(s)||"648".equals(s)||"652".equals(s)||"653".equals(s)){//扇子 剑
					nan = "6491";
					nv = "6512";
				}
				if ("649".equals(s)||"650".equals(s)||"654".equals(s)||"655".equals(s)){
					nan = "6501";
					nv = "6542";
				}
				if ("651".equals(s)||"656".equals(s)){
					nan = "100009";
					nv = "100006";
				}
				if ("750321".equals(s)||"750322".equals(s)||"657".equals(s)||"658".equals(s)){//琴 木筝
					nan = "6561";
					nv = "6562";
				}
				if ("750323".equals(s)||"750324".equals(s)||"659".equals(s)||"660".equals(s)){//琴 木筝
					nan = "6601";
					nv = "6602";
				}
				if ("750325".equals(s)||"661".equals(s)){
					nan = "100007";
					nv = "100008";
				}
				if ("662".equals(s)||"663".equals(s)||"750326".equals(s)||"750327".equals(s)){//男女书
					nan = "6641";
					nv = "6642";
				}
				if ("664".equals(s)||"665".equals(s)||"750328".equals(s)||"750329".equals(s)){//男女书
					nan = "6621";
					nv = "6622";
				}
				if ("666".equals(s)||"750330".equals(s)){
					nan = "6651";
					nv = "6652";
				}
			}
		...
		if (lingbao.getTianfuskill()!=null&&!lingbao.getTianfuskill().equals("")) {
			...
			for (int i = 0; i < tfs.length; i++) {
				...
				if (tfs[i].endsWith("契合")) {
					...
					this.yuanzhu+=0.1*lvl;
				}else if (tfs[i].endsWith("活跃")) {
					this.huoyue*=(1+0.1*lvl);
				}else if (tfs[i].endsWith("抵抗")) {
					this.kangluobao+=0.1*lvl;
				}else if (tfs[i].endsWith("敏捷")) {
					this.sp+=(lvl==2?300:200);
				}else if ((lx==2&&tfs[i].endsWith("增强"))||(lx==1&&tfs[i].endsWith("回生"))) {
					if (tfs[i].indexOf("根骨")!=-1) {
						this.shanghai+=0.0005*lvl*(int)(manData.getHuoyue()/4);
					}else if (tfs[i].indexOf("灵性")!=-1) {
						this.shanghai+=0.0005*lvl*(int)(manData.getShanghai()/4);
					}else if (tfs[i].indexOf("力量")!=-1) {
						this.shanghai+=0.0005*lvl*(int)(manData.getKangluobao()/4);
					}else if (tfs[i].indexOf("敏捷")!=-1) {
						this.shanghai+=0.0005*lvl*(int)(manData.getYuanzhu()/4);
					}
				}
			}
		}
	...
	public void JQ(double xs){
		...
		this.Quality.addkang((int)(xs*2));
		this.Quality.addQ(xs*2);
		this.Quality.addHS(xs*2);
...
//////////////////////////////////////////////////////////////////////////// 武神山守护 /////////////////////////////////////////////////////////
	...
	public ManData(int id, String name, JSONObject ren, double xs) {
		...
		for (int i = 0 ; i < jineng.size() ; i++) {
			Skill skill = GameServer.getSkill(jineng.getString(i));// 根据技能id获取
			if (skill != null) {
				FightingSkill fightingSkill = new FightingSkill(skill,this.lvl, 3,this.zs * 5000 + 10000, 2000000, (int)xs);
				if (id>=100&&id<=104) {
					fightingSkill.setSkillsum(10);
					fightingSkill.setSkillcontinued(100);
					if (id == 104) {
...
//////////////////////////////////////////////////////////////////////////武神山守护  END/////////////////////////////////////////////////////////
	...
	public ManData(Monster fightMonsterBean, int camp, int man, int lvl) {
		...
		String s = Sepcies_MixDeal.localNameMap.get(Integer.parseInt(fightMonsterBean.getMonsterskin()));
		String[] strings = GetWeapons(s);
		long weaponSkin = getWq(strings[0]);
		long se = Integer.parseInt(fightMonsterBean.getMonsterskin());
...
//		String eSkin=((weaponSkin<<32)|Long.parseLong(fightMonsterBean.getMonsterskin()))+"";
		...
		if (fightMonsterBean.getMonsterskill() != null
			...
			if (this.id>=2000&&this.id<=2105) {sld=1;}
			for (int i = 0; i < v.length; i++) {
				try {
					...
					if (skilltype == 1) {continue;}
						...
						for (int j = 0; j < this.skills.size(); j++) {
							FightingSkill skill2=this.skills.get(j);
		...
		try {
			this.Quality = fightMonsterBean.getQl();
			this.Quality.setKzml(this.Quality.getKzml() + CustomFunction.XS(this.hp_z, 0.4)+ 5);
		...
		//
//		this.Quality.setRolekzd(188);
//		this.Quality.setKzds(10000);
	}
...
//            return new String[]{"灯笼", "灯笼"};
	...
	public void neidang(String type,int lvl){
		for (int i = 0; i < this.addStates.size(); i++) {
			if (this.addStates.get(i).getStatename().indexOf(type)!=-1) {
				if (this.addStates.get(i).getStateEffect()<lvl) {
					this.addStates.get(i).setStatename(type+lvl);
					this.addStates.get(i).setStateEffect(lvl);
				}
				return;
			}
		}
	...
	public void ChangeData(ChangeFighting change, FightingState state) {
		...
		if (change.getChangehp() < 0) {
			...
			if (this.type == 0) {
				...
				if (skill != null) {
					int maxhp = (int) (this.hp_z * skill.getSkillhurt() / 100.0);
				...
				if (this.huoyue >= 700) {
					...
					if (skill != null) {
						int maxhp = (int) (this.hp_z * (100-skill.getSkillgain()) / 100.0);
						if (-change.getChangehp() > maxhp) {
							change.setChangehp(-maxhp);
						}
					}
			...
			if (this.type == 0 && change.getSkill(9283) == null) {
...
//	    	9804	浪遏飞舟	沧波状态下的目标有{公式一}%几率不能获得加攻和加速状态。
		...
		if (types.equals(TypeUtil.YW)) {
			...
			for (int i = 0; i < this.skills.size(); i++) {
				if (this.skills.get(i).getSkillbeidong() == 1) {
					continue;
				}
				int skillId = this.skills.get(i).getSkillid();
			...
			if (lx == 0) {
				...
				if (types.equals(TypeUtil.L_LL)) {
					UPModel(state, CreepsMixdeal.getL_LL(this.model));
				}
				UP(state, 0, addState.getStateEffect() / 100D);
			}
		} else {
	...
	public static String[] values2 = { "遗忘", "封印", "中毒", "昏睡", "混乱",TypeUtil.FB_JGE, TypeUtil.FB_QW,TypeUtil.BB_LHFM,TypeUtil.BB_DHSM,TypeUtil.BB_SS,TypeUtil.BB_百草竞发};
	...
	public void RemoveAbnormal(String... values) {
		for (int i = this.addStates.size() - 1; i >= 0; i--) {
			String statename = this.addStates.get(i).getStatename();
	...
	public boolean chongfustate(AddState addState) {
		for (int i = 0; i < this.addStates.size(); i++) {
			if (this.addStates.get(i).getStatename().equals(addState.getStatename())) {
	...
	public AddState xzstate(String type) {
		if (this.addStates == null)
			return null;
		for (int i = 0; i < this.addStates.size(); i++) {
			if (this.addStates.get(i).getStatename().equals(type)) {
				return this.addStates.get(i);
			}
		}
		return null;
	}
	...
	public ManData bb(String bbid) {
		...
		if (id != -1) {
			for (int i = 0; i < this.pets.size(); i++) {
				if (this.pets.get(i).getPlay() == 1) {
					this.pets.get(i).setPlay(2);
				}
				if (id == this.pets.get(i).getHang().getId().intValue()) {
					this.pets.get(i).setPlay(1);
					petData = this.pets.get(i).getPet();
				}
			}
		}
		return petData;
	...
	public List<String> skilluse() {
		...
		for (int i = 0; i < this.skills.size(); i++) {
			if (this.skills.get(i).getSkillbeidong() == 0
					&& !SkillCooling(this.skills.get(i).getSkillid())) {
				s.add(this.skills.get(i).getSkillname());
			}
		}
		return s;
	}
	...
	public boolean zhiding(PathPoint point) {
		if (point.getX() == this.camp && point.getY() == this.man) {
			return true;
		}
		return false;
	}
	...
	public boolean isLicense(FightingSkill skill) {
		if (skill.getSkillid() >= 5001 && skill.getSkillid() <= 5015) {
			if (this.yqz >= skill.getSkillblue()) {
				return true;
			} else {
				return false;
			}
		} else if (skill.getSkillid() >= 9000 && skill.getSkillid() <= 9999) {
			if (this.nqz >= skill.getSkillblue()) {
				return true;
			} else {
				return false;
			}
		} else if (skill.getSkillname().equals("兵临城下")) {
			if (this.mp >= (int) (mp_z * getBlcxKc(2)) && this.hp > (int) (this.hp_z * getBlcxKc(1))) {
				return true;
			} else {
				return false;
			}
		} else if (skill.getSkillname().equals("背水一战")
	...
	public double getk(String type) {
		...
		if (type.equals("普通攻击")) {
			...
			if (addState != null) {
				xiao += (addState.getStateEffect()*0.251);
			}
...
//		if (attack != null && Battlefield.isV(100)) {//修罗内丹几率调试
...
//				if (Battlefield.random.nextInt(100) < 100) { //孩子技能触发几率调试
					return skill;
				}
			}
		}
