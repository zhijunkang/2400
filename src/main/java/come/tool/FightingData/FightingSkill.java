...
public class FightingSkill implements Cloneable{
	...
	public FightingSkill(Talent talent,int lvl){
		...
		this.skilllvl=lvl;
		this.skillhitrate=talent.getTouch()*lvl;
		String[] v=talent.getValue().split("\\|");
		String[] v1=v[0].split("=")[1].split("\\+");
		this.skillhurt=Double.parseDouble(v1[0])+Double.parseDouble(v1[1])*lvl;
		v1=v[1].split("=")[1].split("\\+");
		this.skillgain=Double.parseDouble(v1[0])+Double.parseDouble(v1[1])*lvl;
	...
	public FightingSkill(Skill skill,int lvl,int born,double sld,long qm,int pz) {
		...
		if (this.skillid>1000&&this.skillid<=1100) {//师门技能
		...
		}else if (this.skillid>=1200&&this.skillid<1300) {//召唤兽天生技能
		...
		}else if (this.skillid==8043) {//落井下石（主动）减少对方单人300点怨气
			this.skilltype=this.skillid+"";
			this.skillsum=1;
			...
			this.skillgain=40;
			this.skillbeidong=1;
		} else if (this.skillid==8037||this.skillid==8038||this.skillid==8041||this.skillid==8042||this.skillid==8045||this.skillid>=8060&&this.skillid<=8065||(this.skillid>=8053&&this.skillid<=8059)) {//特技
			this.skillbeidong=1;
			this.skilltype = this.skillid + "";
		...
		}else if (this.skillid>=9000&&this.skillid<=9999) {//天演策
			this.skilltype=this.skillid+"";
			this.skillbeidong=1;
			...
			//单数被动，双数主动
			if(this.skillid == 22001 ||this.skillid == 22003||this.skillid == 22005||this.skillid == 22007||this.skillid == 22009
					||this.skillid == 22011 ||this.skillid == 22013||this.skillid == 22015||this.skillid == 22017||this.skillid == 22019
					||this.skillid == 22021 ||this.skillid == 22023||this.skillid == 22025||this.skillid == 22027||this.skillid == 22029
					||this.skillid == 22031 ||this.skillid == 22033||this.skillid == 22035
			) {
				this.skillbeidong=1;//都是被动
			}

		}else if(skillid>23000&&skillid<23010) {
			if(skillid==23001) {
				this.skillbeidong=1;
	...
	public void ShumenSkill(int lvl,double value,double sv,double sld){
		...
		if (this.skillid>=1001&&this.skillid<=1005) {//混乱法术
			...
			//命中概率
			this.skillhitrate= value + sv * BigDecimal.valueOf(Math.pow(sld, 0.3)).setScale(2, RoundingMode.HALF_UP).doubleValue();
			//命中个数
			this.skillsum=geshu(this.skilllvl,(int)sld,this.skilltype);
			//总生命百分比伤害
//			this.skillhurt=100000;
			//  0.9  自己调吧 我也不清楚官方计算上海的 只知道记几率是对的
			this.skillhurt=value + sv * BigDecimal.valueOf(Math.pow(sld, 0.9)).setScale(2, RoundingMode.HALF_UP).doubleValue();
			//毒伤
			this.skillgain=(lvl+sld/100)*8+(this.skilllvl==5?10000:this.skilllvl==4?12000:8000);
		}else if (this.skillid>=1021&&this.skillid<=1025) {//震慑
			this.skilltype="震慑";
			//命中个数
			this.skillsum=geshu(this.skilllvl,(int)sld,this.skilltype);
			//当前生命百分比伤害
			this.skillhurt=(double)((int)((value+sld*sv/1000)*1000))/10;
		}else if (this.skillid>=1026&&this.skillid<=1030) {//力量增益
			this.skilltype="力量";
			this.skillcontinued=7;//持续回合
			//命中个数
			this.skillsum=geshu(this.skilllvl,(int)sld,this.skilltype);
		...
		}else if ((this.skillid>=1031&&this.skillid<=1035)) {//抗性
			this.skilltype="抗性";
			this.skillcontinued=7;//持续回合
			//命中个数
			this.skillsum=geshu(this.skilllvl,(int)sld,this.skilltype);
			//附加状态振幅百分比
			this.skillgain=(double)((int)((value+sld*sv/1000)*1000))/10;
		}else if (this.skillid>=1036&&this.skillid<=1040) { //加速
			this.skilltype="加速";
			//持续回合
		...
		}else if (this.skillid>=1066&&this.skillid<=1070) {//三尸虫
			this.skilltype="三尸虫";
			//   命中个数
			this.skillsum=geshu(this.skilllvl,(int)sld,this.skilltype);
			//伤害
		...
		}else if (this.skillid>=1071&&this.skillid<=1075) {//遗忘性
			this.skilltype="遗忘";
			//持续回合
			this.skillcontinued=7;
			//	    命中概率
...
//			this.skillhurt=value+sv*new BigDecimal(Math.pow(sld, 0.3)).setScale(2,BigDecimal.ROUND_HALF_UP).doubleValue();
			this.skillhurt=value+sv*new BigDecimal(Math.pow(sld, 0.25)).setScale(2,BigDecimal.ROUND_HALF_UP).doubleValue();
			this.skillgain=this.skilllvl==5?30:this.skilllvl==4?35:25;
...
//			this.skillhurt=value+sv*new BigDecimal(Math.pow(sld, 0.3)).setScale(2,BigDecimal.ROUND_HALF_UP).doubleValue();
			this.skillhurt=value+sv*new BigDecimal(Math.pow(sld, 0.25)).setScale(2,BigDecimal.ROUND_HALF_UP).doubleValue();
			this.skillgain=this.skilllvl==5?30:this.skilllvl==4?35:25;
			this.skillhitrate=this.skilllvl==5?20:this.skilllvl==4?25:15;
			this.skillcontinued=3;
			this.skillsum=geshu(this.skilllvl,(int)sld,this.skilltype);
...
//			this.skillhurt=value+sv*new BigDecimal(Math.pow(sld, 0.3)).setScale(2,BigDecimal.ROUND_HALF_UP).doubleValue();
			this.skillhurt=value+sv*new BigDecimal(Math.pow(sld, 0.25)).setScale(2,BigDecimal.ROUND_HALF_UP).doubleValue();
			this.skillgain=this.skilllvl==5?30:this.skilllvl==4?35:25;
			this.skillhitrate=this.skilllvl==5?15:this.skilllvl==4?20:10;
			this.skillcontinued=3;
			this.skillsum=geshu(this.skilllvl,(int)sld,this.skilltype);
		}
		this.skillblue=(int)(this.skillblue*((sld/25000)+1));
	}
...
//				return;
			...
			case 1241://百草竞发
				this.skilltype = String.valueOf(this.skillid);
				this.skillgain = 40 + CustomFunction.XS(qm, 1);
				this.skillhurt = 100 + CustomFunction.XS(qm, 0.7);
				this.skillbeidong = 1;
				this.skillcontinued = 2;
				return;
			case 1244:// 剑荡八荒
	...
	public void PetSkill(double value,double sv,int lvl,int born,long qm){
		if (skillid==1833||skillid==1832) {
			...
			skillbeidong=1;
			this.skilltype="闪现";
			this.skillhitrate=40+CustomFunction.XS(qm, (skillXls.getValue()/100));
			return;
