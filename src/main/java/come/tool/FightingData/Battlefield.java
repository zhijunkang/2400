			...
			for (int j =  data.getSkills().size()-1; j >=0; j--) {
				...
				if (id>=1041&&id<=1065) {
					...
					if (skill.getSkilltype().equals(TypeUtil.BB_HW)) {
						...
						if (this.chaosSp>sp) {this.chaosSp=sp;}
						data.getSkills().remove(j);
...
//		星阵属性 星阵属性=星阵名称=星阵的五行属性=宫位属性星卡
	...
	public int getBuffid() {return ++this.buffid;}
	...
	public boolean addBuff(GroupBuff buff){
		...
		for (int i = this.buffs.size()-1; i >=0; i--) {
			GroupBuff buff3=this.buffs.get(i);
	...
	public GroupBuff getBuff(int camp,String type){
		...
		for (int i = this.buffs.size()-1; i >=0; i--) {
			GroupBuff buff=this.buffs.get(i);
	...
	public  List<FightingEvents> ReceiveEvent(){
		...
		try {
			...
			for (ManData data:fightingdata)
				data.setHF(CurrentRound);
			this.baohu.clear();
			this.erwai.clear();
			this.huayu.clear();
			this.JLSkills.clear();
			this.recordList.clear();
			if (this.NewEvents==null){this.NewEvents=new ArrayList<>();}
			else{this.NewEvents.clear();}
			this.Events=this.battleData.getPolicyMap().get(this.CurrentRound);
			waitSell();
			RoundDeal();
		} catch (Exception e) {
	...
	public void RoundDeal() throws InterruptedException{
		...
		List<FightingState> hxStates=null;
		for (int i = this.fightingdata.size()-1; i >=0; i--) {
			ManData data=this.fightingdata.get(i);
			if (data.getStates()!=0||data.getType()==3||data.getType()==4){
				 continue;
			}
			FightingSkill skill1=data.getSkillType("1275");
			if (skill1!=null){
				if (data.xzstate(Petdll.Mypetdll.mypetdll.bianshen(5,2))!=null){
					data.deleteState(Petdll.Mypetdll.mypetdll.bianshen(5,2));


				}else if (data.xzstate(Petdll.Mypetdll.mypetdll.bianshen(7,2))!=null){
					data.deleteState(Petdll.Mypetdll.mypetdll.bianshen(7,2));

				}else {
					data.deleteState(Petdll.Mypetdll.mypetdll.bianshen(6,2));


				}

				//String add=data.getHZT()[random.nextInt(data.getHZT().length)];
				data.addAddState(Petdll.Mypetdll.mypetdll.bianshen(random.nextInt(3)+5,2),0,0,3);
				...
				accepter.setEndState_1(Petdll.Mypetdll.mypetdll.bianshen(random.nextInt(3)+5,2));
				Accepterlist.add(accepter);
				zl.setAccepterlist(Accepterlist);
				NewEvents.add(zl);

			}
			FightingSkill skill2=data.getSkillType("1276");
			if (skill2!=null){
				if (data.getHF()%4==0){

					data.addAddState(Petdll.Mypetdll.mypetdll.bianshen(1,5),0,0,2);
					FightingEvents zl=new FightingEvents();
					FightingState accepter=new FightingState();
					accepter.setCamp(data.getCamp());
					accepter.setMan(data.getMan());
					List<FightingState> Accepterlist=new ArrayList<>();
					accepter.setEndState_1(Petdll.Mypetdll.mypetdll.bianshen(1,5));
					accepter.setSkillskin(Petdll.Mypetdll.mypetdll.bianshen(2,5));
					Accepterlist.add(accepter);
					zl.setAccepterlist(Accepterlist);
					NewEvents.add(zl);
					data.UPModel(accepter,Petdll.Mypetdll.mypetdll.bianshen(3,5));
				}else if (data.getstat(Petdll.Mypetdll.mypetdll.bianshen(1,5))==null&&data.getHF()%2==0&&data.getHF()!=2){


					data.addAddState(Petdll.Mypetdll.mypetdll.bianshen(2,5),0,0,2);
					FightingEvents zl=new FightingEvents();
					FightingState accepter=new FightingState();
					accepter.setCamp(data.getCamp());
					accepter.setMan(data.getMan());
					List<FightingState> Accepterlist=new ArrayList<>();
					accepter.setEndState_1(Petdll.Mypetdll.mypetdll.bianshen(2,5));
					accepter.setSkillskin(Petdll.Mypetdll.mypetdll.bianshen(1,5));
					//accepter.setStartState("代价");
					Accepterlist.add(accepter);
					zl.setAccepterlist(Accepterlist);
					NewEvents.add(zl);
					data.UPModel(accepter,Petdll.Mypetdll.mypetdll.bianshen(4,5));
				}
			}
			...
			if (data.getType()==0) {
				...
				addState = data.xzstate("回血");
				if(addState != null ) {
					FightingState fightingState1 = new FightingState();
					if (acStates == null) acStates = new ArrayList<>();
					acStates.add(fightingState1);
					fightingState1.setCamp(data.getCamp());
					fightingState1.setMan(data.getMan());
					fightingState1.setStartState("代价");
					fightingState1.setProcessState("清心静气");
					data.addnq(10, fightingState1);
					if (addState != null) {
						fightingState1.setStartState(TypeUtil.JN);
			...
			if (manData.getStates()==0){
				if (manData.getType()==1&&manData.getMan()>=5&&manData.getMan()<10) {
					...
					if (sk1!=null||sk2!=null) {
						...
						if (path!=-1) {
							ManData data=this.fightingdata.get(path);
							if (sk1!=null) {
								FightingState fightingState=new FightingState();
								fightingState.setStartState(TypeUtil.JN);
...
//	            	//护盾存在期间每个攻击自己的敌方目标都会受到灼烧,处于该状态下的目标下回合开始前随机受到自已气血上限（0.5%*等级）~（1.5%*等级）的伤害,护盾最多持续3回合。(仅在与玩家之间战斗有效
			   ...
			   if (manData.getSkillType("顾影自怜")!=null) {
					if (MixDeal.isgyzl(manData.getCamp(),this)) {
						...
						if (skill!=null&&isV(skill.getSkillgain())&&(skill.getSkillcontinued()==0||this.CurrentRound>=skill.getSkillcontinued()+5)) {
							skill.setSkillcontinued(this.CurrentRound);
		...
		//初始化指令处理
		if (this.CurrentRound==1) {DataActionType.getActionById(32).analysisAction(null, null,null, this);}
		...
		SXBuff();//buff刷新
	    for (int i = this.fightingdata.size()-1; i >=0; i--){
	    	if(this.fightingdata.get(i).getType()==1){
	    		this.fightingdata.remove(i);
	    	}
	    }
	    for (int j = this.fightingdata.size()-1; j >=0; j--) {
			ManData fighting=this.fightingdata.get(j);
	...
	public void ZLCL() {
		...
		while (this.waitList.size()!=0||this.erwai.size()!=0||this.huayu.size() != 0) {
			...
			if (size > 999999) {
				...
				WriteOut.addtxt(1 + ":"+ GsonUtil.getGsonUtil().getgson().toJson(this.Events), 9999);
				WriteOut.addtxt(2 + ":"+ GsonUtil.getGsonUtil().getgson().toJson(this.erwai), 9999);
				WriteOut.addtxt(3 + ":"+ GsonUtil.getGsonUtil().getgson().toJson(this.huayu), 9999);
				break;
			}
			// 先判断战斗是否结束
			if (endFighting(this.fightingdata.get(0).getCamp()) != -1) {
				break;
			}
			if (this.huayu.size() != 0) {
				// 化羽归尘处理
			...
			if (type.equals("防御")||type.equals("保护")) {
				int wei = -1;
				for (int i = 0; i < this.fightingdata.size(); i++) {
					if ( this.fightingdata.get(i).getCamp()==zl.getOriginator().getCamp()&&
							this.fightingdata.get(i).getMan()==zl.getOriginator().getMan()) {
						wei = i;
					}
				}
				if (wei != -1) {
					...
					AddState zt1=myData.xzstate("封印");
					if(zt1!=null) {
						if(type.equals("保护")) {
				...
				if (wei != -1) {
					ManData bbData = this.fightingdata.get(wei).bb(type.split("\\&")[1]);
			...
			if (path == -1) {continue;}
			myData = this.fightingdata.get(path);
			if (myData.getType() == 4) {// 为孩子处理
				...
				if (wei != -1) {
					...
					myData = this.fightingdata.get(wei);
					for (int i = myData.getSkills().size()-1; i >=0; i--) {
						FightingSkill skill2=myData.getSkills().get(i);
				...
				CSGZ(this.fightingdata.get(path), zl);
...
//				}else
			...
			if (this.isWLKH) {HurtWLKH();}//雾里看花处理
			if (this.isHSJY) {HurtHSJY();}//火上浇油处理
	...
	public void CSGZ(ManData myData,FightingEvents zl){
        ...
        if (myData.getAddStates()!=null && (zl.getOriginator().getEndState() == null || !zl.getOriginator().getEndState().equals("流风回雪"))) {
        	for (int i = myData.getAddStates().size()-1; i >=0; i--) {
        		...
        		if (h==0&&Stype.equals(TypeUtil.HL)) {
					...
					return;
				}else if (Stype.equals(TypeUtil.FY)) { this.Events.add(zl);return; }   //封印直接结束
        	}
		}
        if (type.equals(TypeUtil.PTGJ)) {
	    	if (myData.getType()==1) {
				GroupBuff buff=getNoCampBuff(myData.getCamp(), TypeUtil.BB_XLHC);
				if (buff!=null&&this.recordList.contains(TypeUtil.PTGJ+"_"+myData.getCamp())) {return;}
        ...
        if (type.equals("药")) {
        	if (myData.getType()==1) {
				GroupBuff buff=getNoCampBuff(myData.getCamp(), TypeUtil.BB_XLHC);
				if (buff!=null&&this.recordList.contains("药_"+myData.getCamp())) {return;}
        ...
        if (myData.getType()==1) {
			GroupBuff buff=getNoCampBuff(myData.getCamp(), TypeUtil.BB_XLHC);
			if (buff!=null&&this.recordList.contains(skill.getSkilltype()+"_"+myData.getCamp())) {return;}
			this.recordList.add(skill.getSkilltype()+"_"+myData.getCamp());
		}
		...
		this.JLSkills.put(id,myData.getCamp());
		if ((id>=1001&&id<=1020)||(id>=1071&&id<=1075)) {//控制法术
			...
			SpellActionType.getActionById(29).spellAction(myData, skill, zl, this);

		}else if (id==1260) {//新增技能 万佛朝宗
			SpellActionType.getActionById(11).spellAction(myData, skill, zl, this);

		}else if (id==1261) {//新增技能 一刀流
			SpellActionType.getActionById(31).spellAction(myData, skill, zl, this);

		}else if (id==1262) {//新增技能 泰山压顶
			SpellActionType.getActionById(32).spellAction(myData, skill, zl, this);

		}else if (id==1263) {//新增技能 金身不灭
			SpellActionType.getActionById(33).spellAction(myData, skill, zl, this);
		}else if (id==1264) {//新增技能 万箭穿心
			SpellActionType.getActionById(34).spellAction(myData, skill, zl, this);

		}else if (id==1265) {//新增技能 困世法笼
			SpellActionType.getActionById(35).spellAction(myData, skill, zl, this);

		}else if (id==1266) {//新增技能 直捣黄龙
			SpellActionType.getActionById(36).spellAction(myData, skill, zl, this);

		}else if (id==1267) {//新增技能 倒转乾坤
			SpellActionType.getActionById(37).spellAction(myData, skill, zl, this);

		}else if (id==1268) {//新增技能 雷罚
			SpellActionType.getActionById(38).spellAction(myData, skill, zl, this);

		}else if (id==1269) {//新增技能 神罚
			SpellActionType.getActionById(39).spellAction(myData, skill, zl, this);

		}else if (id==1270) {//新增技能 藏锋蓄势
			SpellActionType.getActionById(40).spellAction(myData, skill, zl, this);

		}else if (id==1271) {//新增技能 魔尽佛生
			SpellActionType.getActionById(41).spellAction(myData, skill, zl, this);
		}else if (id==1272) {//新增技能 炎阳神罚
			SpellActionType.getActionById(42).spellAction(myData, skill, zl, this);

		}else if (id==1273) {//新增技能 气化三清
			SpellActionType.getActionById(43).spellAction(myData, skill, zl, this);

		}else if(id==23004) {
			SpellActionType.getActionById(44).spellAction(myData, skill, zl, this);
			...
			if (id==22000||id==22012||id==22032||id==22034||id==22026) {
				SpellActionType.getActionById(24).spellAction(myData, skill, zl, this);
			}
			if(id==22006||id==22016||id==22022) {
				DataActionType.getActionById(2).analysisAction(myData,zl,type,this);
			}
			if(id==22002||id==22004||id==22010||id==22028||id==22030||id==22014||id==22008||id==22018||id==22024||id==22020) {
	...
	public ManData bhpd(int camp,int man){
		for (int i = 0; i < this.baohu.size(); i++) {
			...
			if (state.getCamp()==camp&&state.getMan()==man) {
				int nomypath=Datapathhuo(this.baohu.get(i).getOriginator().getCamp(),this.baohu.get(i).getOriginator().getMan());
				if (nomypath!=-1) {
					this.baohu.remove(i);
	...
	public int xzsx(int camp,int man){
		for (int i = 0; i < this.NewEvents.size(); i++) {
			if (this.NewEvents.get(i).getOriginator()==null||!this.NewEvents.get(i).getOriginator().getStartState().equals("闪现")) continue;
			if (this.NewEvents.get(i).getOriginator().getCamp()==camp&&this.NewEvents.get(i).getOriginator().getMan()==man) {
				return i+1;
			}
		}
		return 0;
	...
	public ManData taopao(int camp,int man){
		for (int i = 0; i < this.fightingdata.size(); i++) {
			if ( this.fightingdata.get(i).getCamp()==camp&&
				this.fightingdata.get(i).getMan()==man&&this.fightingdata.get(i).getType()==0) {
	...
	public int Datapath(int camp,int man){
		for (int i = 0; i < this.fightingdata.size(); i++) {
			if ( this.fightingdata.get(i).getCamp()==camp&&
					this.fightingdata.get(i).getMan()==man&&this.fightingdata.get(i).getStates()!=2) {
				return i;
			}
		}
		return -1;
	...
	public int Datapathhuo(int camp,int man){
		for (int i = 0; i < this.fightingdata.size(); i++) {
			if (this.fightingdata.get(i).getCamp() == camp && this.fightingdata.get(i).getMan()==man && this.fightingdata.get(i).getStates() == 0) {
				return i;
			}
		}
		return -1;
	...
	public int RoundDatapath(int camp,int man){
		for (int i = 0; i < this.Events.size(); i++) {
			if ( this.Events.get(i).getOriginator().getCamp()==camp&&
					this.Events.get(i).getOriginator().getMan()==man) {
				return i;
			}
		}
		return -1;
	...
	public PathPoint instructionspath2(int camp, int noskill, ManData data) {
		...
		while (true) {
			size++;
			if (size>999) {break;}
			int path = random.nextInt(this.fightingdata.size());
	...
	public void waitSell(){
		...
		if (ling1!=null)this.waitList.add(ling1);
		ManData ling2=getling(lingbao2);
		if (ling2!=null)this.waitList.add(ling2);
	...
	public void guilei(){
		...
		if (haizi!=null) {
			...
			for (int i = haizi.size()-1; i >=0; i--) {
				...
				if (skill!=null) {
					...
					data.setSp2(random.nextInt(this.chaosSp)+(int)skill.getSkillhurt());
					List<ManData> datas=MixDeal.get(false, data, 0,nomy(data.getCamp()), 1,0,0,0, 1, 6, this,1);
					if (datas.size()!=0)
						datas.get(0).setSp2(random.nextInt(this.chaosSp)+(int)skill.getSkillgain());
				}
		...
		for (int i = this.Events.size() - 1; i >= 0; i--) {
			...
			if (org.getStartState().equals("防御")) {
				...
				if (org.getEndState().equals("清心静气")||org.getEndState().equals("凝神一击")||org.getEndState().equals("气吞山河")||org.getEndState().equals("行气如虹")||org.getEndState().equals("气聚神凝")||org.getEndState().equals("神龙摆尾")||org.getEndState().equals("知盈处虚")||org.getEndState().equals("流风回雪")||org.getEndState().equals("逆鳞")||org.getEndState().equals("天罡八卦")) {
					this.Events.remove(i);
