	...
	public static void ChangeProcess(ChangeFighting changeFighting,ManData mydata,ManData data,
		...
		if (yylz==0) {
			...
			if (changeFighting.getChangehp()<0) {
				...
				if (type.equals("庇护")||type.equals("盾破")||type.equals("反震")|| type.equals("至圣")||type.equals("怒气")||type.equals("怨气")) {
...
//		 1309	背向	受到伤害时，有{公式三}%几率背向敌人，以减少{公式一}%受到的物理、法术、灵宝伤害。
		...
		if (xs!=100) {
			xs=xs/100.0;
			if (xs<0) {xs=0;}
			int i = (int) (changeFighting.getChangehp() * xs);
			if (i==0){
				i=-1;
			}
			changeFighting.setChangehp(i);
		}
	} 
