...
public class LTSUtil {
	...
	public PathPoint getJF(BigDecimal roleId){
		return this.ltsBean.getLtsMap().get(roleId);
	}
	/**添加积分*/
	public void addJF(BigDecimal roleId,int jf){
//		x:表示总积分 y:表示可用积分
		PathPoint point=this.ltsBean.getLtsMap().get(roleId);
		if (point==null) {
	...
	public void init(){
		...
		if (lts==null||lts.equals("")) {
			...
			this.ltsBean=GsonUtil.getGsonUtil().getgson().fromJson(lts, LTSBean.class);
		}
		bangList(null);
	...
	public void resetLts(List<LoginResult> list){
		...
		if (this.ltsBean.getCi()>=2) {//重置全部积分
	...
	public void bangList(String week){	
		List<Map.Entry<BigDecimal, PathPoint>> infoIds = new ArrayList<Map.Entry<BigDecimal, PathPoint>>(this.ltsBean.getLtsMap().entrySet());
		// 排序
