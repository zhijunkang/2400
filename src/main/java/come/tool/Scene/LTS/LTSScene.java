...
public class LTSScene implements Scene{
	...
	public LTSRole getRole(String role){
		LTSRole ltsRole=this.ltsMap.get(role);
		if (ltsRole==null) {
	...
	public void addJF(PkMatch pkMatch,int type,BigDecimal id){
		...
		if (is) {//刷新排行榜
			SendMessage.sendMessageToMapRoles(this.mapID,Agreement.getAgreement().duelBoradDataAgreement(getRanking()));
		}
	...
	public synchronized boolean PointRanking(LTSRole role){
		...
		boolean is2=false;
		for (int i =0; i<this.top.size() ; i++) {
			LTSRole zzsRole=this.top.get(i);
		...
		if (is) {
			...
			for (int i = this.top.size()-1;i>=5;i--) {
				this.top.remove(i);
				is2=true;
			}
		}
	...
	public LTSArena getArena(PkMatch pkMatch){
		...
		for (int i = 0; i < this.arenas.length; i++) {
			if (this.arenas[i].getPkMatch()!=null) {
				if (this.arenas[i].getPkMatch()==pkMatch) {
					return  this.arenas[i];
				}
			}
		}
	...
	public boolean isLZ(BigDecimal roleId){
		for (int i = 0; i < this.arenas.length; i++) {
			if (this.arenas[i].getPkMatch()!=null) {
				if (this.arenas[i].getPkMatch().getPkMan1().getRoleId().compareTo(roleId)==0) {
					return  true;
				}
			}
		}
	...
	public void getAward(ChannelHandlerContext ctx, LoginResult loginResult) {
		...
		LTSRole ltsRole=this.ltsMap.get(loginResult.getRolename());
		if (ltsRole==null) {
        ...
        if (this.top.contains(ltsRole)) {//获取擂台霸主奖
