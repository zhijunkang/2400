...
public class DNTGScene implements Scene{
	...
	public DNTGScene() {
		...
		this.HOME_TT=new ChangeMapBean();
		this.HOME_TT.setMapid(door.getDoormap());
		...
		this.HOME_HGS=new ChangeMapBean();
		this.HOME_HGS.setMapid(door.getDoormap());
...
//		else if (value==1) {return camp.getK_JS();}
	...
	public DNTGRole getRole(BigDecimal roleId){
		DNTGRole role=this.Camp_TT_Man.get(roleId);
		if (role==null) {role=this.Camp_HGS_Man.get(roleId);}
		return role;
	}
	/**添加阵营玩家*/
...
//			0小怪 1中怪 2大怪  小于3 天庭方怪物
	        ...
	        for (int j = 0; j < 3; j++) {
				...
				this.moveMap.put(movePath.getBh(), movePath);
	...
	public void move(long time){
		...
		for (int i = this.monsterMap.size()-1; i >=0; i--) {
			DNTGMonster monster=this.monsterMap.get(i);
			if (monster.move()) {//到达终点
				...
				if (bean!=null&&bean.getHp().getHp()>0) {
					long hp=bean.getHp().getHp()-monster.getHurt();
					if (hp<0) {hp=0;}
					bean.getHp().setHp(hp);
	...
	public void isDNTG(long time){
		...
		if (time<10800&&this.TT.getDBY().getHp().getHp()>0&&this.HGS.getDBY().getHp().getHp()>0) {
			//大本营还有血 活动不结束
			return;
		}
		if (this.I==4) {
		...
		if (this.TT.getDBY().getHp().getHp()>this.HGS.getDBY().getHp().getHp()) {
			addQJJF(this.Camp_TT_Man, 0.25);
			addQJJF(this.Camp_HGS_Man,0.10);
			chatBean.setMessage("本次大闹天宫结束,获胜方:天庭");
		}else if (this.HGS.getDBY().getHp().getHp()>this.TT.getDBY().getHp().getHp()) {
			addQJJF(this.Camp_HGS_Man, 0.25);
			addQJJF(this.Camp_TT_Man,  0.10);
			chatBean.setMessage("本次大闹天宫结束,获胜方:花果山");
		}else {
			addQJJF(this.Camp_TT_Man, 0.15);
			addQJJF(this.Camp_HGS_Man,0.15);
	...
	public void Function(MapMonsterBean bean,LoginResult loginResult,String msg){
		...
		int robotType=bean.getRobotType();
		String prompt=null;
		if (msg.equals(this.functions[0])) {//攻击    101  102-104   111  112-114  120 121   130
			if (robotType==101||robotType==111||robotType==120||robotType==121) {
				if (dntgRole.getCamp()!=(robotType==101||robotType==120?0:1)) {//可攻击
					if (robotType==101&&this.TT.getBoosXS()==3) {
						prompt="最少需要破一个箭塔才能攻击首领";
					}else if (robotType==111&&this.HGS.getBoosXS()==3) {
						prompt="最少需要破一个箭塔才能攻击首领";
					}
					prompt=isJG(teams);
					if (prompt==null) {
				...
				if (this.dntg_NV_Monster==null||this.dntg_NV_Monster.getBean()==null||this.dntg_NV_Monster.getBean().getHp().getHp()<=0) {//可攻击
					prompt="活动已经结束";
				}else {
					prompt=isJG(teams);
					if (prompt==null) {
		...
		}else if (msg.equals(this.functions[1])) {//送我到上古战场
			
		}else if (msg.equals(this.functions[2])||msg.equals(this.functions[3])) {//送我进去  送我出去
		...
		}else if (msg.equals(this.functions[4])) {//我要回长安
			ChangeMapBean changeMapBean=new ChangeMapBean();
	...
	public String UPMonster(BattleData battleData, String[] teams, int type,StringBuffer buffer) {
    	...
    	if (type==2) {
    		if (bean.getRobotType()==120||bean.getRobotType()==121) {
    			...
    			if (this.dntg_NV_Monster!=null&&this.dntg_NV_Monster.getBean().getHp().getHp()>0) {
					long hp=bean.getHp().getHp()-1;
					if (hp<0) {hp=0;}
					bean.getHp().setHp(hp);
					XGHPRecord(20);
					...
					if (log!=null) {
						...
						if (dntgRole!=null) {
							DNTG_NV_Ranking ranking=dntgRole.getCamp()==0?this.dntg_NV_Monster.getTTRanking():this.dntg_NV_Monster.getHGSRanking();
	...
	public Map<Integer, MonsterMoveBase> getMapMonster(StringBuffer buffer,Map<Integer, MonsterMoveBase> moveMap, long mapId) {
		...
		if (DNTGScene.mapIdZ==mapId) {
			...
			for (int i = this.monsterMap.size()-1; i >=0; i--) {
				DNTGMonster dntgMonster=this.monsterMap.get(i);
	...
	public synchronized int XGHPRecord(int v){
		...
		this.HPRecord=this.HPRecord|(1<<v);
		return this.HPRecord;
	}
	@Override
	public boolean isEnd() {
		// TODO Auto-generated method stub
		if (this.I==4) {return false;}
		return true;
	}
