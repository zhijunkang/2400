...
public class BWZScene implements Scene{
	...
	public BWZScene() {
		...
		this.day = rightNow.get(Calendar.DAY_OF_WEEK);// 获取时间
		init();
		// 发起世界喊话
	...
	public boolean isEnd(){
		if (this.I==2) {//结束
			return false;
		}else if (this.CI>=30&&this.monsterMap.size()==0) {
			this.I=2;
			return false;
	...
	public void move(){
		...
		for (int i = this.monsterMap.size()-1; i >=0; i--) {
			BWZMonster monster=this.monsterMap.get(i);
...
//	/**刷怪*/
	public void open(){
		...
		for (int i = 0; i < this.btyPaths.length; i++) {
			open(getBoos(i), this.btyPaths[i],i,buffer);
		}
		// 根据地图发送信息
	...
	public Map<Integer, MonsterMoveBase> getMapMonster(StringBuffer buffer,Map<Integer, MonsterMoveBase> moveMap, long mapId) {
		for (int i = this.monsterMap.size()-1; i >=0; i--) {
			BWZMonster bwzMonster=this.monsterMap.get(i);
