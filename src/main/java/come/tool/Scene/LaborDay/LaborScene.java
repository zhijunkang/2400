...
public class LaborScene {
	...
	public static String[] CBTS={};//藏宝图转盘数据
	public static String CZVALUE;//仙玉充值转盘数据  
	public static String CBTVALUE;//藏宝图转盘  
	...
	public static void getCBT() {
		String btjp = "";
		//获取1最好奖励1个
		Dorp dorp = GameServer.getDorp("9001");
//		Draw draw1 = GameServer.getDraw(9001);
		String[] dr1 = dorp.getDorpValue().split("\\|");
		int d1 = SuitComposeAction.random.nextInt(dr1.length);
		btjp = btjp+dr1[d1]+"|";
		//获取2最好奖励1个
		Dorp dorp1 = GameServer.getDorp("9002");
//		Draw draw2 = GameServer.getDraw(9002);
		String[] dr2 = dorp1.getDorpValue().split("\\|");
		int d2 = SuitComposeAction.random.nextInt(dr2.length);
		btjp = btjp+dr2[d2]+"|";
		//获取3最好奖励2个
		Dorp dorp2 = GameServer.getDorp("9003");
//		Draw draw3 = GameServer.getDraw(9003);
		String[] dr3 = dorp2.getDorpValue().split("\\|");
		for(int i=0;i<2;i++) {
			int d3 = SuitComposeAction.random.nextInt(dr3.length);
			btjp = btjp+dr3[d3]+"|";
		}
		//获取最次奖励10个
		Dorp dorp3 = GameServer.getDorp("9004");
//		Draw draw4 = GameServer.getDraw(9004);
		String[] dr4 = dorp3.getDorpValue().split("\\|");
		for(int i=0;i<10;i++) {
	...
	public static int awardValue(){
		...
		if (v<=4)        { v=100000; }
		else if (v<=25)  { v=50000; }
		else if (v<=100) { v=30000; }
		else if (v<=250) { v=10000; }
		else if (v<=550) { v=5000; }
		else             { v=3000; }		
		return v;
	}
	...
	/**根据金额获取位置并且加上随机参数*/
	public synchronized static int awardLocationCBT(int v){
		CZSize++;
		CZValue+=v;
		int location=-1; 
		...
		if (location==-1) { return location; }
		return location+CBTS.length*(CZSize%3==0?3:2);
	}
	...
	public void initLabor(){
		...
		if (this.rankMap==null) { this.rankMap=new ConcurrentHashMap<>(); }
		if (this.roleMap==null) { this.roleMap=new ConcurrentHashMap<>(); }
		Iterator<BigDecimal> it = this.roleMap.keySet().iterator();
		while(it.hasNext()) {
			BigDecimal roleId=it.next();
			LaborRole laborRole=this.roleMap.get(roleId);
	...
	public int getRank(int type,LaborRole role){
		...
		synchronized (laborScene) {
			if (type==0) {
				if (this.CZRankList.size()!=0&&value<this.CZRankList.get(this.CZRankList.size()-1).getCz()) {return 0;}
				for (int i=0,length=this.CZRankList.size();i<length;i++) {
					LaborRole laborRole=this.CZRankList.get(i);
					if (laborRole.getRoleId().compareTo(role.getRoleId())==0) {return i+1;}
				}
			}else if (type==1) {
				if (this.CJRankList.size()!=0&&value<this.CJRankList.get(this.CJRankList.size()-1).getCj()) {return 0;}
				for (int i=0,length=this.CJRankList.size();i<length;i++) {
					LaborRole laborRole=this.CJRankList.get(i);
					if (laborRole.getRoleId().compareTo(role.getRoleId())==0) {return i+1;}
				}
			}
		}
	...
	public void resetRank(int type,LaborRole role){
		if (type==0) {
			if (this.CZRankList.size()<100||role.getCz()>=this.CZRankList.get(this.CZRankList.size()-1).getCz()) {
				this.CZRankList.remove(role);
				for (int i=0,length=this.CZRankList.size();i<length;i++) {
					LaborRole laborRole=this.CZRankList.get(i);
					if (role.getCz()>laborRole.getCz()) {
						this.CZRankList.add(i, role);
						for (int j=100;j<=length;j++) {this.CZRankList.remove(100);}
						return;
					}
				}
				if (this.CZRankList.size()<100) { this.CZRankList.add(role); }	
			}
		}else if (type==1) {
			if (this.CJRankList.size()<100||role.getCj()>=this.CJRankList.get(this.CJRankList.size()-1).getCj()) {
				this.CJRankList.remove(role);
				for (int i=0,length=this.CJRankList.size();i<length;i++) {
					LaborRole laborRole=this.CJRankList.get(i);
					if (role.getCj()>laborRole.getCj()) {
						this.CJRankList.add(i, role);
						for (int j=100;j<=length;j++) {this.CJRankList.remove(100);}
						return;
					}
				}
				if (this.CJRankList.size()<100) { this.CJRankList.add(role); }	
			}
		}
	}
	/**充值排行榜结算*/
	public void resetCZ(){
		synchronized (DRAWLOCK){
			for (int i=0,length=this.CZRankList.size();i<length;i++) {
				LaborRole laborRole=this.CZRankList.get(i);
				this.rankMap.put((1L<<32)|laborRole.getRoleId().longValue(), (i+1L));
			}
		}
	}
	/**每日凌晨重置接口*/
		...
		synchronized (laborScene) {
			...
			synchronized (DRAWLOCK){
				if(this.rankMap != null) {
					...
					for (int i=0,length=this.CJRankList.size();i<length;i++) {
						LaborRole laborRole=this.CJRankList.get(i);
						this.rankMap.put(laborRole.getRoleId().longValue(), (i+1L));
					}
				}
			}
			if(this.CJRankList != null) {
				this.CJRankList.clear();//清空抽奖
			}
...
//		if (I==0) { return Agreement.getAgreement().PromptAgreement("活动未开启"); }
		...
		if(status) {
			...
			//发送系统提示
			String msg = ":25|"+fs[1]+"|1|#SDFSDFSD4WDFSGSDFERTDF454512";
			//判断刷物资
