...
public class ZZSScene implements Scene{
	...
	public void upI(int ST){
		...
		if (this.I==2) {
			...
			SendMessage.sendMessageToMapRoles(this.mapID,msg);
		}else if (this.I==3) {
			NChatBean bean=new NChatBean();
			bean.setId(5);
		...
		for (ZZSRole value : this.zzsMap.values()) { 
			if (isMatch(value)) {
				...
				for (int i =0; i<this.matchs.size() ; i++) {
					ZZSRole role=this.matchs.get(i);
...
//		小到大排序
		...
		for (int i =0; i<size;) {
			...
			i++;
			if (i>=size) {break;}
			ZZSRole role2=this.matchs.get(i);
	...
	public boolean isMatch(ZZSRole value){
		...
		if (log.getMapid()!=this.mapID) {value.setI(0);return false;}
	...
	public boolean isEnd(ZZSRole value){
		...
		if (log.getMapid()!=this.mapID) {return false;}
		return true;	
	}
	/**2个玩家进入战斗*/
	...
	public void BattleEnd(String m1,String m2,int type){
		if (m1!=null) {
			ZZSRole role1=this.zzsMap.get(m1);
			if (role1!=null) {
		...
		if (m2!=null) {
			ZZSRole role2=this.zzsMap.get(m2);
			if (role2!=null) {
	...
	public synchronized void PointRanking(ZZSRole role){
		...
		for (int i =0; i<this.top.size() ; i++) {
			ZZSRole zzsRole=this.top.get(i);
		...
		if (is) {
			...
			for (int i = this.top.size()-1; i >10; i--) {
				this.top.remove(i);
			}
		}
	}
	...
	public ZZSRole getRole(LoginResult loginResult){
		return this.zzsMap.get(loginResult.getRolename());
	}
	@Override
...
//		400-404
	...
	public void clearRole(){
		...
		for (ZZSRole value : this.zzsMap.values()) { 
			if (!this.top.contains(value)) {
				...
				if (ctx!=null) {
					LoginResult log=GameServer.getAllLoginRole().get(ctx);
					if (log!=null&&log.getMapid()==this.mapID) {
						action.action(ctx,GangBattleAction.OUTCA);			
					}	
				}
			}else {
	...
	public void isTT(){//判断当前地图是否只剩一个人
		...
		for (ZZSRole value : this.zzsMap.values()) { 
			if (isEnd(value)) {
				this.matchs.add(value);
				if (this.matchs.size()>=2) {break;}
			}
		}
		if (this.matchs.size()<=1) {
			if (this.matchs.size()==1) {
				ZZSRole role=this.matchs.get(0);
				role.addAward(5);
				NChatBean bean=new NChatBean();
				bean.setId(5);
				StringBuffer buffer=new StringBuffer();
				...
				buffer.append(this.type==0?"人":this.type==1?"魔":this.type==2?"仙":"鬼");
				buffer.append("的冠军");
