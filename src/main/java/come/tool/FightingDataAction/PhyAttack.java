		...
		while (nomyData!=null&&g<maxg) {
			...
			if (g>1&&skill8!=null&&Battlefield.isV(skill8.getSkillhurt())) {
				...
				if (bb_e==1||skill3!=null||skill11!=null||!Battlefield.isV(dsl+dsjc-manData.getQuality().getRolefmzl()-mzjc-manData.mz+manData.executeYszd(3) - manData.mz)) {
					...
					for (String xin:manData.yinian){
						...
						if (xin.equals("圣佑")&&Battlefield.random.nextInt(100)<28){
							...
							for (int i = gsdatas.size()-1; i>=0; i--) {
								...
								if (manData.getSkin().equals("728")||manData.getSkin().equals("729")){
					...
					if (manData.getSkillType("1274")!=null)
						if (gsdatas!=null)
							for (int i = gsdatas.size()-1; i>=0; i--) {
								ManData bjData=gsdatas.get(i);
								ChangeFighting bjacec=new ChangeFighting();
								FightingState bjace=new FightingState();
								ap=Hurt((int)(manData.getAp()),g,manData,bjData,TypeUtil.PTGJ,null,zls,null,0,0);
								if (manData.getSkin().equals("728")||manData.getSkin().equals("729")){//圣猿特效素材
					...
					if (baodata==null) {
						...
						}else if (skill88!=null&&Battlefield.isV(50)&&nomyData.getHp_z()*0.5>nomyData.getHp()&&nomyData.getKangluobao()>=500) {//新增技能 大威天龙 未保护的目标才能被致死
							FightingPackage.ChangeProcess(acec, bb_e==2?null:manData, nomyData, ace, TypeUtil.ZSSH, zls, battlefield);
			...
			if (bb_e!=2&&skill3==null&&skill_9==null&&guiwei.size()==0&&nomyData.getStates()==0&&manData.getStates()==0){
				...
				if (f<maxf) {
					...
					fjc.setChangehp((int)-nomyData.getAp());
//					fjc.setChangehp((int)-ap);
			...
			if(!sb&(nomyData.getQuality().getLffj()>0||nomyData.getQuality().getFffj()>0||nomyData.getQuality().getHffj()>0||nomyData.getQuality().getSffj()>0)) {
				...
				if(fjjl>=random.nextInt(100)) {
					...
					changeFighting.setChangehp(-manData.getMp_z()*2);
					FightingPackage.ChangeProcess(changeFighting, null, manData, fightingState, lei, zls, battlefield);
...
//				int sfhqmm = (int) nomyData.getQuality().getSfhqmm();
//				int sfmsfs = (int) nomyData.getQuality().getSfmsfs();
//				int sfqkjs = (int) nomyData.getQuality().getSfqkjs();
//				if(sfhqmm>=random.nextInt(100)) {
//					lei = "盘";
//					skillId = "1035";
//					FightingState fightingState=new FightingState();
//					fightingState.setStartState("代价");
...
//					fightingState.setSkillskin(skillId);
//				}else if(sfmsfs>=random.nextInt(100)) {
//					lei = "牛";
//					skillId = "1030";
//					FightingState fightingState=new FightingState();
//					fightingState.setStartState("代价");
...
//					FightingPackage.ChangeProcess(changeFighting, null, nomyData, fightingState, lei, zls, battlefield);
//					fightingState.setSkillskin(skillId);
//				}else if(sfqkjs>=random.nextInt(100)) {
//					lei = "速";
//					skillId = "1040";
//					FightingState fightingState=new FightingState();
//					fightingState.setStartState("代价");
...
//				}
	...
	public static void drrlsz(String type,ManData manData,ManData nomyData,long hurt,Battlefield battlefield,List<FightingState> zls){
		...
		ChangeFighting changeFighting=new ChangeFighting();
		changeFighting.setChangemp((int)(-hurt*0.3));//掉蓝
...
//				int Hurt=Calculation.getCalculation().SMHurt(manData,nomyData,fightingSkill.getSkillhurt(),0,fightingSkill.getSkilltype(),manData.getCamp()==1?battlefield.MyDeath:battlefield.NoDeath);
				int Hurt=Calculation.getCalculation().petHurt(manData,nomyData,fightingSkill.getSkillhurt(),0,fightingSkill.getSkilltype(),manData.getCamp()==1?battlefield.MyDeath:battlefield.NoDeath);
		...
		if (attack!=null) {
			...
			if (Battlefield.isV(skill.getSkillgain()+gsjc)) {
				...
				for (int i = gsdatas.size()-1; i>=0; i--) {
					...
					ap = (long)(ap * 1);
					ap-=nomyData.getQuality().getK_ndgs();
					if (luoshu)
						ap= (long) (ap*1)/2;
//					    ap-=nomyData.getQuality().getK_ndgs();
					bjacec.setChangehp((int)-ap);
		...
		//附加攻击攻击判断
		if (z==0) {
			for (int i = 0; i < 10; i++) {//0-3仙法 4毒 5混乱 6封印 7震慑 8三尸
				double jc=0;
				String lei = null;
				String id = null;;
...
//						int Hurt=Calculation.getCalculation().xianfa(manData,nomyData,MixDeal.addition(fightingState,skill.getSkillhurt(),manData, lei),  lei,manData.getCamp()==1?battlefield.MyDeath:battlefield.NoDeath);
						...
						ChangeFighting changeFighting=new ChangeFighting();
						int sh=Calculation.getCalculation().sssh(manData, nomyData,MixDeal.addition(fightingState, manData.getAp(),manData, type));
						sh= (int) ((int)manData.getHp()*0.02);//幻方回血
						changeFighting.setChangehp(-sh);
						FightingPackage.ChangeProcess(changeFighting, null, nomyData, fightingState, TypeUtil.SSC, zls, battlefield);
						id="1069";
						fightingState.setSkillskin(id);

						int Vampire=Calculation.getCalculation().sshx(manData, nomyData, 2500, sh);
						List<ManData> datas=SSCAction.minhp(manData.getCamp(),1,battlefield);
						for (int j = 0; j<1&&j<datas.size(); j++) {
							FightingState f2=new FightingState();
							ManData data=datas.get(j);
							...
							changeFighting.setChangehp(sh*10);
							data.ChangeData(changeFighting, f2);
							f2.setStartState("药");
							zls.add(f2);
						}
