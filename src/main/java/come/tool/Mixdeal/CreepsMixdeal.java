...
//	灯笼:17 弓:18
   /**根据物品皮肤获取武器皮肤*/
    ...
    public static String getL_LL(String model){
		if (model.indexOf("24001")!=-1 || model.indexOf("4294991297")!=-1 || model.indexOf("25769827777")!=-1) {
			return "51101";
		}else if (model.indexOf("24002")!=-1 || model.indexOf("42949696962")!=-1 || model.indexOf("51539631554")!=-1) {
			return "51102";
		}else if (model.indexOf("24003")!=-1 || model.indexOf("77309435331")!=-1 || model.indexOf("47244664259")!=-1) {
			return "51103";
		}else if (model.indexOf("24004")!=-1 || model.indexOf("12884925892")!=-1 || model.indexOf("38654729668")!=-1) {
			return "52101";
		}else if (model.indexOf("24005")!=-1 || model.indexOf("77309435333")!=-1 || model.indexOf("51539631557")!=-1) {
			return "52102";
		}else if (model.indexOf("24006")!=-1 || model.indexOf("4294991302")!=-1 || model.indexOf("73014468038")!=-1) {
			return "52103";
		}
    	return model; 	
    }
    ...
    //非抗性型内丹获取几率、伤害率的方法
  	public static FightingSkill accessNedanMsg(String goodsname,int nddj,int ndzscs,int zhsdj,int zhszscs,long zhsqm,long l){
  		if(goodsname.equals("浩然正气")){
  			...
  			double ndcd=Math.floor((zhsdj*zhsdj*0.2/(l*1+1)+ndjl)*10000)/10000;
  			return new FightingSkill(goodsname, Math.round(ndjl*10000)/100D, Math.round(ndcd*10000)/100D);
  		}else if(goodsname.equals("隔山打牛")){
  			double ndjl=Math.floor((Math.pow(zhsdj*nddj*0.04,1.0/2.0)*(1+0.25*ndzscs)+Math.pow(zhsqm,1.0/6.0)*xstz(zhszscs,ndzscs)*nddj/50)*1000)*0.000005;
  			double ndcd=Math.floor((zhsdj*zhsdj*0.2/(l*1+1)+ndjl*3)*1000)/1000;
  			return new FightingSkill(goodsname, Math.round((ndjl*1000)*0.7)/10D, Math.round((ndcd*1000)*0.779)/10D);
  			...
  			double ndhq=Math.floor(ndjl*(zhsdj*zhsdj*0.15/(l*1+0.01)+0.2)*1000)/1000;
  			...
  			double ndcd=Math.floor(296.1572+0.0002364957*Math.pow(l,1.57));
