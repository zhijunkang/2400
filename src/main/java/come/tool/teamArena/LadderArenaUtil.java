        ...
        synchronized (bean) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                if (teamRole.getState() < 0 || loginResult == null) {
                    ...
                    return;
                } else if ((roleData.getTaskWC(5) - roleData.getTaskLQ(5)) >= 20) {  //修改失败次数
    ...
    public static void teamArenaDraw(TeamBean team, TeamArenaMatch match, Dorp dorp, int add) {
        ...
        for (int i = 0; i < ids.length; i++) {
            ...
            if (loginResult != null) {//在线奖励结算
                ...
                String data = "K=" + add;
                String msg = (add > 0 ? ("你增加了" + add + "点段位积分") : ("你损失了" + (-add) + "点段位积分")) + ",你当前段位为" + getTilte(jf);
...
//                if (roleData.getTaskWC(6) <= 10) {//修改全民组队竞技奖励次数
