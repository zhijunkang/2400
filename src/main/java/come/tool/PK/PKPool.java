...
public class PKPool {
	...
	public PkMatch seekPkMatch(BigDecimal pkMan1){
		return this.PkMatchId.get(pkMan1);
	}
	/**添加挑战者*/
	...
	public PkMatch getPkMatch(BigDecimal pkMan1,BigDecimal pkMan2){
		PkMatch match=this.PkMatchs.get(pkMan1+":"+pkMan2);
		if (match==null) {match=this.PkMatchs.get(pkMan2+":"+pkMan1);}
		if (match==null) {match=this.PkMatchs.get(pkMan1.toString());}
		if (match==null) {match=this.PkMatchs.get(pkMan2.toString());}
		return match;
	}
	//删除
	public PkMatch deletePkMatch(BigDecimal pkMan1,BigDecimal pkMan2){
		PkMatch match=this.PkMatchs.remove(pkMan1+":"+pkMan2);
		if (match==null) {
			match=this.PkMatchs.remove(pkMan2+":"+pkMan1);
		}
		if (match==null&&pkMan1!=null) {
			match=this.PkMatchs.remove(pkMan1.toString());
		}
		if (match==null&&pkMan2!=null) {
			match=this.PkMatchs.remove(pkMan2.toString());
		}
		if (match!=null) {
	...
	public void returnStake(RoleCard pkMan,PKStake pKStake,String ts){
		...
		if (loginResult==null) {
			 ...
			 buffer.append("你获得"+(pKStake.getCharge()+pKStake.getMoney())+"金钱，包含已投入金钱。");
		}
		if (pKStake.getXianYu()!=0) {
			...
			if (loginResult==null) {
				...
				if (loginResult!=null) {
				 ...
				 buffer.append("你获得"+(pKStake.getXianYu())+"仙玉，包含已投入仙玉。");
			}
		}
		pKStake.qk();
	...
	public void OVERTIME() {
		...
		while (it.hasNext()) {
			key = it.next();
			PkMatch value = this.PkMatchs.get(key);
			if (value != null) {
