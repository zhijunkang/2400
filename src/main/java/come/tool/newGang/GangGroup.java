...
public class GangGroup {
	...
	public boolean addXY(int size){
		if (this.xy==0) {return false;}
		int max=(3+this.xy*2)*6;
		if (this.xyNum>max) {return false;}
		this.xyNum+=(3+this.xy*2)*size;
		if (this.xyNum>max) {this.xyNum=max;}
		return true;
	}
//	0|1|1=0
	public String getTxt(){
		if (this.xy==0&&this.kj==0) {return null;}
	...
	public String getXYNpc(){
		...
		if (this.xy==0) {
			...
			buffer.append(this.xyNum);
			buffer.append("(上限:");
			buffer.append((3+this.xy*2)*6);
			buffer.append(")。每小时恢复");
			buffer.append((3+this.xy*2));
