			...
			for (int j = datas.size()-1; j>=0; j--) {
				...
				if (Accepter==null) {
					...
					if (i == sum - 1  && bcjf != null) {  //&& j == datas.size() - 1
						...
						if (zt1 == null && Battlefield.isV(63.5)) {
							...
							ChangeFighting changeFighting2=new ChangeFighting();
							changeFighting2.setChangehp(-(int)(fashang *0.30));
							FightingState fState2=new FightingState();
							FightingPackage.ChangeProcess(changeFighting2, null, data, fState2, "额外", Accepterlist, battlefield);
							manData.RemoveAbnormal(TypeUtil.BB_百草竞发);
						}
					}
