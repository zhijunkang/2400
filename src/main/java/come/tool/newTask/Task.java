...
public class Task {
	...
	public void addProgress(String[] values){
		if (this.progress!=null) {this.progress.clear();}
		else {this.progress=new ArrayList<>();}
	...
	public int PartFinish(int id,int sum,String leixing){
		for (int i = 0; i < this.progress.size(); i++) {
			TaskProgress Progre=this.progress.get(i);
	...
	public boolean isFinish(){
		if (this.taskState==TaskState.doTasking) {
			if (this.progress==null) {
				this.taskState=TaskState.completeTask;
				return true;
			}
			for (int i = 0; i < this.progress.size(); i++) {
				if (this.progress.get(i).getSum()<this.progress.get(i).getMax()) {
					return false;
				}
			}
			this.taskState=TaskState.completeTask;
	...
	public Task FZ(){
		...
		if (this.progress!=null) {
			...
			for (int i = 0; i < this.progress.size(); i++) {
				task.progress.add(new TaskProgress(this.progress.get(i)));
			}
		}
		return task;
	}
