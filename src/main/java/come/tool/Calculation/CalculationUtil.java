...
public class CalculationUtil {
    ...
    public static void initGradeQl() {
        String[] kx = {"抗混乱", "抗封印", "抗昏睡", "抗中毒", "抗风", "抗火", "抗雷", "抗水", "命中率", "狂暴率", "致命率", "物理吸收", "抗遗忘", "抗三尸", "抗鬼火", "抗浩然正气", "躲闪率", "雷法反击", "风法反击", "水法反击", "火法反击", "被攻击时释放魔神附身", "被攻击时释放含情脉脉", "被攻击时释放乾坤借速"};
...
//	400
    ...
    public static void loadRoleBattle(ManData manData, LoginResult login, RoleData roleData, Map<String, Double> roleMap, Map<String, Double> map, Map<String, Double> eMap, List<BaseEquip> equips, BattleData battleData) {
        ...
        addBaseQl(map, roleData.getXls(42));//星盘属性加成
        addBaseRaceKangXing(roleMap,login.getRace_id());//人物基础属性加成
...
//        for (int i = 0; i < roleData.getFs().size() && i < 2; i++) {// 原版法宝
            for (int i = 0; i < roleData.getFs().size() && i < 3; i++) {// 改版法宝
...
//			if (equipType!=0&&qhv>=12) {
        ...
        double ZBMAX = getValue(login.getRace_id(), "装备抗性上限", map, eMap, roleMap, 75) + 75;
        Ql ql = new Ql();
    ...
    public static double getValue(BigDecimal raceId, String key, Map<String, Double> map, Map<String, Double> eMap, Map<String, Double> roleMap, double ZBMAX) {
        ...
        } else if (key.equals("抗三尸") || key.equals("抗浩然正气") || key.equals("抗青面獠牙") || key.equals("抗天魔解体") || key.equals("抗小楼夜哭") || key.equals("抗隔山打牛") || key.equals("抗分光化影")|| key.equals("抗中毒伤害")|| key.equals("抗毒伤")) {

        } else if (key.startsWith("抗") || key.equals("物理吸收")) {
            equip = equip > (100 + max) ? (100 + max) : equip;
        } else if (key.equals("连击率")) {
            return equip + additional + role > (75 + max) ? (75 + max) : equip + additional + role;
...
//		LID$LVL&ID$LVL
    ...
    public static void getPet(Ql ql, RoleSummoning pet, Mount mount, ManData data) {
        ...
        try {
            ...
            if (StringUtils.isNotEmpty(pet.getPetSkills())) {
                split = pet.getPetSkills().split("\\|");
                if (StringUtils.isNotBlank(pet.getBeastSkills()) && "1814".equals(pet.getBeastSkills())) {
                    String[] t = {pet.getBeastSkills()};
                    split = (String[]) ArrayUtils.addAll(split, t);
                }
                skillData = pet.getSkillData();
            }

            if (split != null && split.length >= 2 && StringUtils.isNotEmpty(skillData)) {
                List<String> skillsIds = new ArrayList<>();

                for (String s1 : split) {
                    if (s1.equals("1814") && s1.equals("1803") && s1.equals("1885") && s1.equals("1827"))//  ||为生效其中一个，&&为同时生效
                        skillsIds.add(s1);
                }
                if (skillsIds.size() >= 2) {
                    for (String skillsId : skillsIds) {
                        UsePetAction.subSkillData(skillsIds, skillData);
                    }
                    List<String> oneSkill = new ArrayList<>();
                    if (skillsIds.contains("1814")) {
                        oneSkill.add("1814");
                        skillData = UsePetAction.skillData(oneSkill);
                    } else if (skillsIds.contains("1885")) {
                        oneSkill.add("1885");
                        skillData = UsePetAction.skillData(oneSkill);
                    } else if (skillsIds.contains("1827")) {
                        oneSkill.add("1827");
                        skillData = UsePetAction.skillData(oneSkill);
                    } else if (skillsIds.contains("1803")) {
                        oneSkill.add("1803");
                        skillData = UsePetAction.skillData(oneSkill);
                    }
                }
...
//            if (skill2 != null) {//专属觉醒
//
//                if (skill2.getSkillId() == 1300||skill2.getSkillId() == 1301||skill2.getSkillId() == 1302||skill2.getSkillId() == 1303
//                  ||skill2.getSkillId() == 1304||skill2.getSkillId() == 1305||skill2.getSkillId() == 1306||skill2.getSkillId() == 1307
//                  ||skill2.getSkillId() == 1308||skill2.getSkillId() == 1309||skill2.getSkillId() == 1310||skill2.getSkillId() == 1311
//                  ||skill2.getSkillId() == 1312||skill2.getSkillId() == 1313||skill2.getSkillId() == 1314||skill2.getSkillId() == 1315
//                  ||skill2.getSkillId() == 1316||skill2.getSkillId() == 1332||skill2.getSkillId() == 1333||skill2.getSkillId() == 1334){
//
//                }else {
//                    if (skill2.getSkillId() == 1321 && ("200099".equals(pet.getSummoningid()) || "200195".equals(pet.getSummoningid())) ||
//                        skill2.getSkillId() == 1320 && ("200098".equals(pet.getSummoningid()) || "200195".equals(pet.getSummoningid())) ||
//                        skill2.getSkillId() == 1317 && ("200117".equals(pet.getSummoningid()) || "200195".equals(pet.getSummoningid())) ||
//                        skill2.getSkillId() == 1318 && ("200116".equals(pet.getSummoningid()) || "200195".equals(pet.getSummoningid())) ||
//                        skill2.getSkillId() == 1319 && ("200123".equals(pet.getSummoningid()) || "200195".equals(pet.getSummoningid())) ||
//                        skill2.getSkillId() == 1322 && ("200100".equals(pet.getSummoningid()) || "200195".equals(pet.getSummoningid())) ||
//                        skill2.getSkillId() == 1323 && ("200101".equals(pet.getSummoningid()) || "200195".equals(pet.getSummoningid())) ||
//                        skill2.getSkillId() == 1324 && ("200097".equals(pet.getSummoningid()) || "200195".equals(pet.getSummoningid())) ||
//                        skill2.getSkillId() == 1325 && ("200093".equals(pet.getSummoningid()) || "200195".equals(pet.getSummoningid())) ||
//                        skill2.getSkillId() == 1326 && ("200156".equals(pet.getSummoningid()) || "200195".equals(pet.getSummoningid())) ||
//                        skill2.getSkillId() == 1327 && ("200147".equals(pet.getSummoningid()) || "200195".equals(pet.getSummoningid())) ||
//                        skill2.getSkillId() == 1328 && ("200136".equals(pet.getSummoningid()) || "200195".equals(pet.getSummoningid())) ||
//                        skill2.getSkillId() == 1329 && ("200144".equals(pet.getSummoningid()) || "200195".equals(pet.getSummoningid())) ||
//                        skill2.getSkillId() == 1330 && ("200134".equals(pet.getSummoningid()) || "200195".equals(pet.getSummoningid())) ||
//                        skill2.getSkillId() == 1331 && ("200146".equals(pet.getSummoningid()) || "200195".equals(pet.getSummoningid())) ||
//                        skill2.getSkillId() == 1335 && ("200188".equals(pet.getSummoningid()) || "200195".equals(pet.getSummoningid()))
//                    ) {
//
//                    } else {
//                        skill2 = null;
