...
//		switch (id) {
//		case 11005:
//			GetqualityUntil.AddR(ql, "抗封印", LingXiUtil.getNumberBySkillId("11005", 1, lv));
//			GetqualityUntil.AddR(ql, "抗混乱", LingXiUtil.getNumberBySkillId("11005", 1, lv));
//			GetqualityUntil.AddR(ql, "抗遗忘", LingXiUtil.getNumberBySkillId("11005", 1, lv));
//			GetqualityUntil.AddR(ql, "抗昏睡", LingXiUtil.getNumberBySkillId("11005", 1, lv));
//			break;
//		}
	}
	...
	public static FightingSkill accessNedanMsg(Goodstable goodstable,int nddj,int ndzscs,int zhsdj,int zhszscs,long zhsqm,int zhsmpz){
		...
		if(goodstable.getGoodsname().equals("浩然正气")){
			...
			return new FightingSkill(goodstable.getGoodsname(), Math.round((ndjl*1000)*0.7)/10D, Math.round((ndcd*1000)*0.779)/10D);
