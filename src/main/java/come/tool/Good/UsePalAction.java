...
//			}
    ...
    public void palUPGood(Pal pal,Goodstable good,ChannelHandlerContext ctx,LoginResult login){
    	...
    	while (exp>maxExp) {
            if (lvl==60||lvl==100||lvl==140||lvl==180||lvl==300) {
 				break;
 			}
 	    	exp-=maxExp;
 	    	lvl++;
	...
	public void palJJGood(Pal pal,Goodstable good,ChannelHandlerContext ctx,LoginResult login){
		...
		assetUpdate.upmsg ("你的伙伴:#R"+palData.getName()+"#Y品质变更为"+(nGrow==1?"#G资质平平":nGrow==1.5?"#B出类拔萃":nGrow==2?"#c800080神通广大":nGrow==2.5?"#cFF7F00万中无一":""));
		SendMessage.sendMessageToSlef(ctx,Agreement.getAgreement().assetAgreement(GsonUtil.getGsonUtil().getgson().toJson(assetUpdate)));  	
	}
