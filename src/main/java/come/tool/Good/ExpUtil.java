...
//		//获取当前等级生的最大等级
		...
		impl.updateRoleWhenExit(loginResult);
	}
	/**依据经验进行升级判断,内丹升级*/
...
//				ZhuFrame.getZhuJpanel().addPrompt2("内丹等级不可超过召唤兽等级");
	...
	public static void MountMove(Mount mount,int addexp){
		//判断等级最大能升多少级
		int moveGrade = mount.getMoveGrade()+addexp;
		mount.setMoveGrade(moveGrade);
	}
	...
	public static void FlyExp(Fly fly,int addexp){
		int grade=fly.getFlylvl();
		int exp=fly.getExp()+addexp;
		long maxexp=getFlyExp(grade);
		while ((exp>maxexp)){
			if (grade==100){break;}
			grade++;
			maxexp=getFlyExp(grade);
		}fly.setExp(exp);
		if (grade>fly.getFlylvl()){
			fly.setFlylvl(grade);
		}
	}
