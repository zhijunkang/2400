...
public class UsePetAction implements IAction {
	...
	public void action(ChannelHandlerContext ctx, String message) {
		...
		if (type == 715) {//715    亲密丹
			...
			if (value >= Integer.parseInt(configure.getZhsqmsx())) {
				value = Integer.parseInt(configure.getZhsqmsx());
			}
			...
			petCzUpDan(pet, good, ctx, loginResult);
	...
	public static void petTransformAppearance(RoleSummoning pet, Goodstable good, ChannelHandlerContext ctx, LoginResult login) {
		//心猿限制使用玲珑宝鉴
			if (Integer.parseInt(pet.getSummoningskin())<=729 && Integer.parseInt(pet.getSummoningskin())>=721 ) {
			SendMessage.sendMessageToSlef(ctx, Agreement.getAgreement().PromptAgreement("#R警告：该召唤兽不能随意变换造型！"));
			return;
		}
		//心猿限制使用玲珑宝鉴
		AssetUpdate assetUpdate = new AssetUpdate();
...
//				}
//			}
			if (pet.getOpenSeal() <= skills.size() || skills.size() >= 4) {//召唤兽技能7-8格自动领悟
				return false;
			}
...
//		if (Integer.parseInt(pet.getSsn())<2) {
//			SendMessage.sendMessageToSlef(ctx, Agreement.getAgreement().PromptAgreement("这只召唤兽不能使用成长进阶丹!!!"));
//			return;
//		}
		if (pet.getTurnRount() < 1) {// 判断是否为未转召唤兽
			SendMessage.sendMessageToSlef(ctx, Agreement.getAgreement().PromptAgreement("召唤兽 " + pet.getSummoningname() + "未转生不可服用成长进阶丹！"));
			return;
		}
		if (pet.getTurnRount() < 2) {// 判断是否为一转召唤兽
			if(pet.getCzjjd()>=20) {
				SendMessage.sendMessageToSlef(ctx, Agreement.getAgreement().PromptAgreement("召唤兽 " + pet.getSummoningname() + "的进阶次数已达到上限！一转上限20个"));
				return;
			}
		}
		if (pet.getTurnRount() < 3) {// 判断是否为二转召唤兽
			if(pet.getCzjjd()>=30) {
				SendMessage.sendMessageToSlef(ctx, Agreement.getAgreement().PromptAgreement("召唤兽 " + pet.getSummoningname() + "的进阶次数已达到上限！二转上限30个"));
				return;
			}
		}
		if (pet.getTurnRount() < 4) {// 判断是否为三转召唤兽
			if(pet.getCzjjd()>=50) {
				SendMessage.sendMessageToSlef(ctx, Agreement.getAgreement().PromptAgreement("召唤兽 " + pet.getSummoningname() + "的进阶次数已达到上限！三转上限50个"));
				return;
			}
		}
		if (pet.getTurnRount() < 5) {// 判断是否飞升
			if(pet.getCzjjd()>=100) {
				SendMessage.sendMessageToSlef(ctx, Agreement.getAgreement().PromptAgreement("召唤兽 " + pet.getSummoningname() + "的进阶次数已达到上限！飞升后可增加上限到100颗！"));
				return;
			}
		}
		if (pet.getCzjjd() >= 100) {
			SendMessage.sendMessageToSlef(ctx, Agreement.getAgreement().PromptAgreement("召唤兽 " + pet.getSummoningname() + "的进阶次数已达到上限！召唤兽最多可吃100颗"));
			return;
		}
		AssetUpdate assetUpdate = new AssetUpdate();
		assetUpdate.setType(AssetUpdate.USEGOOD);
		...
		AllServiceUtil.getRoleSummoningService().updatePetRedis(pet);
		assetUpdate.setPet(pet);
		assetUpdate.setMsg("召唤兽 " + pet.getSummoningname() + "进阶成功！！！");
		SendMessage.sendMessageToSlef(ctx, Agreement.getAgreement().assetAgreement(GsonUtil.getGsonUtil().getgson().toJson(assetUpdate)));
	}
...
//		200123 白泽 200116 年 200117画中仙 200097 垂云叟 200098 范式之魂 200099 浪淘沙 200100 五叶 200101 颜如玉 200184 灵听 200185 飞轩 200188 连生 200189 大吕
		int petid = Integer.parseInt(pet.getSummoningid());
		if (petid != 200123 && petid != 200116 && petid != 200117 && petid != 200097 &&
				petid != 200098 && petid != 200099 && petid != 200100 && petid != 200101 && petid != 200184 && petid != 200185 && petid != 200188 && petid != 200189) {
			SendMessage.sendMessageToSlef(ctx, Agreement.getAgreement().PromptAgreement("白泽 年 画中仙 颜如玉 垂云叟 五叶 范式之魂 灵听 飞轩 莲生 大吕才能使用化形丹"));
			return;
		}
		AssetUpdate assetUpdate = new AssetUpdate();
		assetUpdate.setType(AssetUpdate.USEGOOD);
...
//		2转50级6  272 2转100级7 322 2转140级8 362 3转70级9  433
	...
	public void useKeelsp(RoleSummoning pet, Goodstable good, ChannelHandlerContext ctx, LoginResult login) {
		...
		if(configure!=null) {
			if (pet.getSpdragon() >= Integer.parseInt(configure.getCjlzgsx().toString())) {// 不超过2个
...
//		}
		...
		while (true) {// 随机给这只召唤兽的hp、mp、ap、sp随机加6点
			...
			if (isCJ) {
				...
				switch (GameServer.random.nextInt(4)) {
					case 0:ran1=100;break;
					case 1:ran2=100;break;
					case 2:ran3=100;break;
					case 3:ran4=100;break;
				}
			}
			if (ran1 + ran2 + ran3 + ran4 == 100) {
				pet.setHp(pet.getHp() + ran1);
				pet.setMp(pet.getMp() + ran2);
				pet.setAp(pet.getAp() + ran3);
				pet.setSp(pet.getSp() + ran4);
	...
	public static void addPetSkill(RoleSummoning pet,Goodstable good,ChannelHandlerContext ctx,LoginResult login) {
		...
		if (sum<=skills.size()||skills.size()>=Integer.parseInt(configure.getZhsjngs())) {//召唤兽技能格子已经满了
	...
	public static void addpetqiling(RoleSummoning pet,Goodstable good,ChannelHandlerContext ctx,LoginResult login) {
		...
		pet.setSkillData(skillData(skillList));
		AllServiceUtil.getRoleSummoningService().updatePetRedis(pet);
		assetUpdate.setPet(pet);
		assetUpdate.setMsg("你的召唤兽学会了"+skill.getSkillname());
		int Id=skill.getSkillid();
	...
	public static String chongfu(Skill skill,RoleSummoning pet,List<String> lists,boolean l) {
		...
		if (id==1820) {// 金50
			...
			if (pet.getPower()<450) {return "你的召唤兽力量不足450";}
		} else if (id==1831) {//
			if (pet.getPower()<450) {return "你的召唤兽力量不足450";}
		}else if (id==1839) {//
	...
	public static String subSkillData(List<String> skills,String skilldata){
		{
//			if(s1.equals("1814") || s1.equals("1803") ||s1.equals("1885") || s1.equals("1827"))
			// 获取所有的技能id
			for (int i = 0; i < skills.size(); i++) {
				switch (skills.get(i)) {
					case "1803":
