...
public class TSModel {
	...
	public DropType getDropType(int type){
		...
		for (int i = 0; i < this.types.size(); i++) {
			if (this.types.get(i).getDropType()==type) {
	...
	public MonsterMatch getMatch() {
		if (this.match!=null) {return new MonsterMatch(this.match.getCountDown());}
		return this.match;
	}
	public MonsterExp getExp() {
		if (this.exp!=null) {return new MonsterExp(this.exp.getMaxSize(), this.exp.getExp());}
		return this.exp;
	}
}
