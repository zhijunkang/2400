...
public class DropUtil {
	...
	public static AssetUpdate getDrop(LoginResult login,DropModel model,String msg,AssetUpdate assetUpdate,double expXs1,double expXs2,int num,int type){
		if (model==null) {return assetUpdate;}
		...
		if (ctx==null||roleData==null) {return assetUpdate;}
		...
		if (model.getExp()!=null||model.getExpFix()!=null) {//经验
			long exp=0;
			if (model.getExp()!=null) {exp+=model.getExp().longValue()*expXs1;}
			if (model.getExpFix()!=null) {exp+=model.getExpFix().longValue();}
				...
				if (dropType.getDropType()==DropType.SCORE) {//积分=xxx
					...
					if (model.getMaxGood()!=null&&model.getMaxGood()<num) {
						...
						if (bean!=null) {
							...
							//特效物品判断是拥有特效
							if (id.longValue()<0&&roleData.getPackRecord().isTX(-id.longValue()+"")) {continue;}
					...
					if (gangDomain!=null) {gangDomain.addBG(dropType.getValue());}
				}
			}
		}
		if (buffer.length()!=0) {
...
//							SuitMixdeal.good(goodstable,login.getRolename(), msg,type);
		...
		Configure configure = s.get(5);

		NChatBean bean=new NChatBean();
		bean.setId(4);
		bean.setMessage("#Y尊贵的#G月卡#Y玩家#G【"+login.getRolename()+"】#Y领取了月卡礼包【"+configure.getZqsld()+"】！");
		String msgt = Agreement.getAgreement().chatAgreement(GsonUtil.getGsonUtil().getgson().toJson(bean));
		SendMessage.sendMessageToAllRoles(msgt);
		if (buffer.length()!=0) {
			if (assetUpdate==null) {assetUpdate=new AssetUpdate(AssetUpdate.USEGOOD);}
			assetUpdate.upmsg(buffer.toString());
		}
		...
		if (model.getTypes()!=null&&(model.getMaxDrop()==null||model.getMaxDrop()>lvl)) {
			for (int i = 0; i < model.getTypes().size(); i++) {
				...
				if (dropType.getDropType()==DropType.SCORE) {//积分=xxx
					...
					buffer.append(value);
					GangDomain gangDomain=GangUtil.getGangDomain(login.getGang_id());
					if (gangDomain!=null) {gangDomain.addBG(value);}
				}
			}
		}
		if (buffer.length()!=0) {
			...
			if (thing[0].equals("物品")) {
				if (is&&roleData.isGoodFull()) {
					...
					if (bean!=null) {
						...
						if (id.longValue()<0&&roleData.getPackRecord().isTX(-id.longValue()+"")) {continue;}
						if( goodstable == null ){continue;}
						if (assetUpdate==null) {assetUpdate=new AssetUpdate(type);}
						if (buffer.length()!=0) {buffer.append("|");}
						buffer.append("你获得");
						buffer.append(bean.getSum());
						buffer.append("个");
...
//							assetUpdate.setMsg("你已经拥有称谓:"+title.getTitlename());
				...
				if (gangDomain!=null) {gangDomain.addBG(money);}
			}
		}
		if (assetUpdate!=null) {
			if (message!=null) {assetUpdate.upmsg(message);}
			if (buffer.length()!=0) {assetUpdate.upmsg(buffer.toString());}	
			if (data!=null) {assetUpdate.setVip(data);}
			if (dataMes!=null) {assetUpdate.updata(dataMes);}
			if (task!=null) {assetUpdate.setTask(task);}
	...
	public static XXGDBean getGoods(DropGood dropGood,int addEmpty) {
		if (dropGood.getEmpty()>0) {
			if (addEmpty==0) {
				if (isV(dropGood.getEmpty())) {return null;}	
			}else {
				if (dropGood.getEmpty()>=70) {addEmpty/=3;}
				else if (dropGood.getEmpty()>=50) {addEmpty/=2;}
				else if (dropGood.getEmpty()<=20) {addEmpty*=1.5;}
				if (isV(dropGood.getEmpty()+addEmpty)) {return null;}	
			}	
		}
	...
	public static boolean isDH(String ab,LoginResult loginResult){
		...
		if (!vs[vs.length-1].equals("#SDFSDFSD4WDFSGSDFERTDF454512")) {return false;}
		ChannelHandlerContext ctx=GameServer.getRoleNameMap().get(loginResult.getRolename());
		XXGDBean bean=new XXGDBean();
		bean.setId(vs[1]);
