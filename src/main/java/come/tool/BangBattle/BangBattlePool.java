...
public class BangBattlePool {
    ...
    public void init(){
        ...
        if (text.equals("")){
        	...
        	this.group=GsonUtil.getGsonUtil().getgson().fromJson(text, Group.class);
            this.group.init(0);	
		}
        //分组 
...
//		FightOpenClose();
    ...
    public BangFight getBangFight(BigDecimal bang_id){
		...
		for (int i = this.BangFights.size()-1; i >=0; i--) {
			if (this.BangFights.get(i).getMap(bang_id)!=null)
				return this.BangFights.get(i);
		}
    	return null;
    }
	...
	public BangPoints getBangPoints(BigDecimal id){
		...
		for (int i = this.group.getlist().size()-1; i >=0; i--) {
			if (this.group.getlist().get(i).getId().compareTo(id)==0){
				return this.group.getlist().get(i);	
			}
		}
		return null;		
	}
		...
		if (victory==null&&fail==null) {
			if (group.getlist().size()>1) {
				for (int i = (group.getlist().size()-1)/2; i >=0; i--) {
					...
					this.BangFights.add(new BangFight(sum,Left,Right));
				}	
			}						
		}else {
			Match(victory,sum+1);
	...
	public void FightOpenClose(){
		...
		for (int i = 0; i < this.BangFights.size(); i++) {
			this.BangFights.get(i).BangState=1;
		}
//		bangThread.notify();
	}
	//胜负处理
