...
import org.come.action.qly.QLYAction;
import org.come.action.role.*;
import org.come.action.sale.*;
...
import come.tool.Good.UseFlyAction;
...
public class ParamTool {
	...
	public static void handles() {
		...
		// qly
		QLYAction qlyAction = new QLYAction();
		ACTION_MAP.put(AgreementUtil.rolePath, qlyAction);
		//科举
		IAction action12=new KejuAction();
		ACTION_MAP.put(AgreementUtil.kejunpc,action12);
		//科举回答
		IAction action13=new KejuhuidaAction();
		ACTION_MAP.put(AgreementUtil.kejuhuida,action13);
		//添加飞行器
		IAction action666=new FlyGetAction();
		ACTION_MAP.put(AgreementUtil.flyget,action666);
		//飞行器使用物品变化
		IAction action209=new UseFlyAction();
		ACTION_MAP.put(AgreementUtil.usefly,action209);
		// 获得角色宝宝
		...
		IAction action1000 = new itemAction();
		ACTION_MAP.put(AgreementUtil.item, action1000); 
		// 投放赏功堂物品
        ...
        GoodsExchangeAction action999 = new GoodsExchangeAction();
        ACTION_MAP.put("GOODSFORGOODS", action999);
        ACTION_MAP.put("findDrop", new FindDropAction());
        ACTION_MAP.put("hjsl", new HjslAction());
        ACTION_MAP.put("lingxi", new LxAction());
        ACTION_MAP.put("getEgg", new EggAction());
		// 灵犀操作
...
//		ACTION_MAP.put(AgreementUtil.ACCOUNT_GOLOGIN, action554);
		// 签到
		ACTION_MAP.put(AgreementUtil.QD, new QianDaoAction());
		//GMshop发送
		ACTION_MAP.put(AgreementUtil.GMSHOP, new GMshopItemAction());
		//泡点数据发送
		ACTION_MAP.put(AgreementUtil.expAdd, new expAddAction());
		//全民竞猜
		ACTION_MAP.put(AgreementUtil.QMJC, new LotteryRoleAction());
		//修改仙玉发送
		ACTION_MAP.put(AgreementUtil.updateXianyu, new updateXianyuAction());

		// 卖家发货
		IAction deliverAction = new GoodsDeliverAction();
		ACTION_MAP.put(AgreementUtil.CBGoodDeliver, deliverAction);

		IAction autoSwitchAction = new AutoSwitchAction();
		ACTION_MAP.put(AgreementUtil.autoSwitch, autoSwitchAction);
	}
}