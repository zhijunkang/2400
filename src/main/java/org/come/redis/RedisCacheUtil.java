...
public class RedisCacheUtil {
	...
	private  static  BigDecimal fly_pk;//飞行器ID
	...
	public static RedisCacheBean<?> getRedisCacheBean(String Key){
		...
		else if(Key.equals(RedisParameterUtil.FLY)){return flyBean;}
		return null;
	}
	/**键值*/
	public void InitZhu(){
		...
		fly_pk=getMax(jedis.hkeys(RedisParameterUtil.FLY));
		...
		fly_pk=getGl(fly_pk,GameServer.getQh());
    ...
    private boolean BD6(Fly t1,Fly t2){
		if (t1.getFlylvl().intValue()!=t2.getFlylvl().intValue()){return true;}
	...
	private boolean BD4(Mount t1, Mount t2) {
        ...
        if (t1.getSid4()==null) {if (t2.getSid4()!=null) {return true;}}
        else if (t2.getSid4()==null) {if (t1.getSid4()!=null) {return true;}}
        else if (t1.getSid4().compareTo(t2.getSid4())!=0) {return true;}
        
        if (t1.getSid5()==null) {if (t2.getSid5()!=null) {return true;}}
        else if (t2.getSid5()==null) {if (t1.getSid5()!=null) {return true;}}
        else if (t1.getSid5().compareTo(t2.getSid5())!=0) {return true;}
...
//    	    if (TIME-time>2592000) {continue;}
			...
			for (int j = 0; j < loginResults.size(); j++) {
				...
				RoleDataBean bean=new RoleDataBean(loginResult, 
						...
						AllServiceUtil.getFlyService().selectFlysByRoleID(loginResult.getRole_id()));
				...
				for (int i = 0; i < transplantBean.getList().size(); i++) {
					...
					for (int j = 0; j < userData.getRoleDataBeans().size(); j++) {
						...
						for (Fly fly:flys){
							...
							RedisControl.insertKey(RedisParameterUtil.FLY,fly.getMid().toString(),GsonUtil.getGsonUtil().getgson().toJson(fly));
							RedisControl.insertListRedis(RedisParameterUtil.FLY,fly.getRoleid().toString(),fly.getMid().toString());
							RedisControl.insertController(RedisParameterUtil.FLY,fly.getMid().toString(),RedisControl.CADD);
