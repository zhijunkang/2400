...
public class AllMeridians {
    ...
    public void setMeridians(List<Meridians> meridiansList) {
        for (Meridians meridians : meridiansList) {
            if (!this.meridiansMap.containsKey(meridians.getId())) {
                this.meridiansMap.put(meridians.getId(), new ArrayList<>());
            }
            this.meridiansMap.get(meridians.getId()).add(meridians);
        }
    }

    public List<Meridians> getByType(int id) {
        return this.meridiansMap.get(id);
    }
    ...
    public Meridians getRandomByType(int id) {
        for (Meridians m : this.meridiansMap.get(id)) {
        ...
        return RandomUtil.randomEle(this.meridiansMap.get(id));
    }

    public Meridians getFirstByType(int id) {
        return this.meridiansMap.get(id).get(0);
    }
