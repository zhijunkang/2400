...
//		加强风#加强火#加强水#加强雷#加强鬼火#加强中毒=2.8|加强昏睡#加强封印#加强混乱#加强遗忘#强震慑#加强攻击法术效果#加强防御法术效果=1.2|加强魅惑=2.4
	...
	public GemBase getGemBase(String type){
		...
		for (int i = this.gemBases.size()-1; i >=0; i--) {
			GemBase base=this.gemBases.get(i);
			if (base.getType().equals(type)) {return base;}
		}
		return this.gemBases.get(0);	
	}
	/**随机一个宝石属性*/
	public GemBase rGemBase(){return this.gemBases.get(Battlefield.random.nextInt(this.gemBases.size()));}
	public int getBid() {
		return this.bid;
	}
