...
//	记录任务完成条件 系列ID=R任务领取次数=L任务完成次数=D任务进度
	...
	public String isLvl(LoginResult[] logs,int size){
		...
		}else if (this.TeamSum!=1&&this.TeamSum>size) {
			return Agreement.getAgreement().PromptAgreement("队伍人数不够"+this.TeamSum+"个人,先凑齐人数在来吧");
		}
		if (this.lvls!=null) {
			for (int i = 0; i < logs.length; i++) {
				String value=BattleMixDeal.isLvl(logs[i].getGrade(), this.lvls);
	...
	public String isSum(boolean is,LoginResult[] logs){
		if (this.taskSet.getSumlimit()==0&&this.taskSet.getSumreceive()==0)return null;
		for (int i = 0; i < logs.length; i++) {
			...
			if (record!=null) {
				...
				}else if (is&&this.taskSet.getSumreceive()!=0&&record.getrSum()>=this.taskSet.getSumreceive()) {
	...
	public Object isConsume(LoginResult[] logs){
		if (this.consumes==null) {return null;}
	...
	public int getNewTaskId() {
		if (this.postTaskIds==null||this.postTaskIds.size()==0) {return 0;}
		return this.postTaskIds.get(GameServer.random.nextInt(this.postTaskIds.size()));
	}
	public int getNd() {
		return this.nd;
	}
