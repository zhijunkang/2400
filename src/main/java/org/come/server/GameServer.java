...
public class GameServer implements ServletContextListener {
	...
	public static int redisPort = 6389;
	/**
	 * 合区时前缀
	 */
...
//		String paths = this.getClass().getClassLoader().getResource("/").getPath() + "serverCode.json";
//		try {
//			String msg = AESUtil.AESJDKDncode(CreateTextUtil.getContent(paths), ">LA~h4FNKPMJW099vd0");
//			mcCode = msg;
//			String msgs = getProperty();
//			if(!msgs.equals(msg)) {
//				return;
//			}
...
//			}
		...
		// boolean check =
		// "54b53072540eeeb8f8e9343e71f28176".equals(AESUtil.getMD5(userName))
		// &&
		// "1a3d5f94c44b49770851080a6c08dc3".equals(AESUtil.getMD5(password));
		// if(check){
		// break;
		// }
		// }
...
//		}
		...
		packgradecontrol = new ConcurrentHashMap<>();
	...
	public static void userDownTwos(ChannelHandlerContext ctx) {
		...
		// 获得全部召唤兽
		List<RoleSummoning> petss=AllServiceUtil.getRoleSummoningService().selectRoleSummoningsByRoleID(exitUser.getRole_id());
		...
		if(loginResult.getBooth_id()!=null) {
			if(!configure.getLxbtkg().equals("开")) {
		...
		if(configure.getSfdkms().equals("1")) {
			...
			Long xh = cd-Long.parseLong(exitUser.getGameStartTime());
			Long second = xh/1000;//计算秒
			Long dk = second/60;
			exitUser.setGameTimeRemaining(Integer.parseInt((roleInfo.getGameTimeRemaining()-dk)+""));
		}
		try {
	...
	public static void setAllMount(ConcurrentHashMap<Integer, ConcurrentHashMap<Integer, Mount>> allMount) {
	  ...
	  public static Fly getFly(int raceid ,int id){
			ConcurrentHashMap<Integer,Fly> map=allFly.get(raceid);
			if (map==null){
				return  null;
			}
			Fly fly=map.get(id);
	  ...
	  public static Keju getkeju(int racid,int id){
			ConcurrentHashMap<Integer,Keju>map=allkeju.get(racid);
			if (map==null){
				return null;
			}
			Keju keju=map.get(id);
			if (keju!=null){
				keju=keju.clone();
			}
			return keju;
	...
	public static Goodstable getgoods(BigDecimal id) {
		Goodstable goods = allitem.get(id);
		if (goods != null) {
			return goods.clone();
		}
		return goods;
	...
	public static void setAllitem(ConcurrentHashMap<BigDecimal, Goodstable> allitem) {
	   ...
	   public static GoodsExchange getGoodsExchange(int id) {
		      return (GoodsExchange)allGoodsExchange.get(Integer.valueOf(id));
		   }


	   public static void setAllGoodsExchange(ConcurrentHashMap<Integer, GoodsExchange> allGoodsExchange) { GameServer.allGoodsExchange = allGoodsExchange; } 
