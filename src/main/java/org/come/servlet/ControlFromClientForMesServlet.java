...
public class ControlFromClientForMesServlet extends HttpServlet {
	...
	public void doPost( HttpServletRequest request, HttpServletResponse response)
		...
		this.requestReturnBean=GsonUtil.getGsonUtil().getgson().fromJson(condition, RequestReturnBean.class);
		if(this.requestReturnBean.getRequresHeader().equals("10006")){
			managerTable manege= GsonUtil.getGsonUtil().getgson().fromJson(this.requestReturnBean.getContent(),managerTable.class);
			this.requestReturnBean=InterfaceControlUntil.controlManagerControl(this.requestReturnBean.getContent(), this.impl, this.serimpl);
            if(manege.getControlStyle()==6666){
            	request.getSession().setAttribute("manger", manege);
            	request.getSession().setAttribute("agentNameList", GsonUtil.getGsonUtil().getgson().fromJson(this.requestReturnBean.getContent(), List.class));}


		}else if(this.requestReturnBean.getRequresHeader().equals("10007")){
			ServiceArea service1= GsonUtil.getGsonUtil().getgson().fromJson(this.requestReturnBean.getContent(),ServiceArea.class);
