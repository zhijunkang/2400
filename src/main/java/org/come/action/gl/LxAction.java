...
public class LxAction implements IAction {
	...
	*/
	...
	private long needMoneyNum = 500000;
    private long needExperienceNum = 1000000;
    private long needQinmiNum = 50000;

    //消耗仙玉
    private long xidian = 1000;
    ...
    public void action(ChannelHandlerContext ctx, String message) {
        ...
        if (vs[0].equals("D")) { //洗点
            //D&SID
            // 扣除玩家货币
            if (loginResult.getCodecard().longValue() < this.xidian) {
                SendMessage.sendMessageByRoleName(loginResult.getRolename(),Agreement.getAgreement().PromptAgreement("#R你的仙玉不足以支付洗点费用#17"));
                return;
            }

            loginResult.setCodecard(new BigDecimal(loginResult.getCodecard().longValue() - this.xidian));
            ...
            assetUpdate.updata("X=-" + this.xidian);;
        ...
        if (vs[0].equals("X")) { //单次修炼
            ...
            // 扣除玩家货币
            if (loginResult.getGold().longValue() < this.needMoneyNum) {
                SendMessage.sendMessageByRoleName(loginResult.getRolename(),Agreement.getAgreement().PromptAgreement("#R你的金钱不足召唤兽本次修炼所需#132"));
                return;
            }


            loginResult.setGold(new BigDecimal(loginResult.getGold().longValue() - this.needMoneyNum));
            MonitorUtil.getMoney().useD(this.needMoneyNum);
            pet.setExp(new BigDecimal(exp - this.needExperienceNum));
            pet.setLingxi(lx);
            pet.setFriendliness(qm - this.needQinmiNum);
            AllServiceUtil.getRoleSummoningService().updatePetRedis(pet);

            AssetUpdate assetUpdate=new AssetUpdate();
            assetUpdate.setType(AssetUpdate.USEGOOD);
            assetUpdate.updata("D=-" + this.needMoneyNum);
            assetUpdate.setPet(pet);
            SendMessage.sendMessageToSlef(ctx,Agreement.getAgreement().assetAgreement(GsonUtil.getGsonUtil().getgson().toJson(assetUpdate)));
        ...
        if (vs[0].equals("Z")) { //一键修炼
            ...
            lx = addXiulian(pet.getSsn(), lx,count);
            if (lx == null) {
                return;
            }
            if (!lx.startsWith("Lx")) {
     ...
     * @return
     */
    public String addXiulian(String ssn, String lingxi, int count) {
        String param[] = lingxi.split("&");
        if (param.length != 4) {
            return null;
        }
        ...
        // 灵犀点数
        int ds = Integer.parseInt(param[2].split("=")[1]);
        if ("2".equals(ssn) ||"3".equals(ssn)  ||"4".equals(ssn)  ||"7".equals(ssn)) {
            if (ds == 110) {
