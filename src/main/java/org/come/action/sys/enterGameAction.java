...
//									SendMessage.sendMessageToSlef(ctx, Agreement.getAgreement().PromptAgreement("#R提示：您的召唤兽#G【"+summoning.getSummoningname()+"】#Y已过期！"));
		...
		String autoSwitch = jedis.hget(AgreementUtil.autoSwitch, roleInfo.getRole_id().toString());
		...
		if (roleInfo.getFighting()!=0) {
			...
			SendMessage.sendMessageToSlef(ctx, Agreement.getAgreement().PromptAgreement("您上次的离开时间为：#G" + sd + "，祝您游戏开心#9"));
			if(roleInfo.getMoney()>100) {
				NChatBean bean=new NChatBean();
				bean.setId(5); 
				if(roleInfo.getMoney()>500) {
					bean.setMessage("#Y尊贵的#G神豪#Y玩家#G【"+roleInfo.getRolename()+"】#Y上线了！江湖又将迎来一场血雨腥风！！！");
				}else if(roleInfo.getMoney()>300) {
					bean.setMessage("#Y尊贵的#G土豪#Y玩家#G【"+roleInfo.getRolename()+"】#Y上线了！江湖又将迎来一场血雨腥风！！！");
				}else {
					bean.setMessage("#Y尊贵的玩家#G【"+roleInfo.getRolename()+"】#Y上线了！祝他玩的开心！");
				}
				...
				NChatBean bean=new NChatBean();
				bean.setId(5); 
				bean.setMessage("#Y萌新玩家#G【"+roleInfo.getRolename()+"】#Y上线了！大家多多帮衬！");
				String msg = Agreement.getAgreement().chatAgreement(GsonUtil.getGsonUtil().getgson().toJson(bean));
				SendMessage.sendMessageToAllRoles(msg);
			}
			if(configure.getSfdkms().equals("1")) {
				if(roleInfo.getGameTimeRemaining()!=null) {
					...
					SendMessage.sendMessageToSlef(ctx, Agreement.getAgreement().PromptAgreement("您的游戏点数不足请及时补充！当前点数【#R"+0+"#Y】"));
				}
			}
			if(petss!=null && petss.size()>0) {
				for(RoleSummoning summoning:petss) {
