...
public class SuitComposeAction implements IAction {
	public static String[] Extras=new String[]{"炼化属性","炼器属性","神兵属性","套装属性","宝石属性","觉醒技", "五行属性","星阵属性","赞助属性","点粹属性" };
	public static String CHECKTS1=Agreement.getAgreement().PromptAgreement("#R开光成功");
	public static String CHECKTS2=Agreement.getAgreement().PromptAgreement("开光失败");
	public static String CHECKTS3=Agreement.getAgreement().PromptAgreement("#R升级成功");
	...
	static{
		...
		YFTJ=new String[17];//衣服特技
		for (int i = 0; i < YFTJ.length; i++) {
			YFTJ[i]=(8018+i)+"";
		}
		XLTJ=new String[16];//项链特技
		for (int i = 0; i < XLTJ.length; i++) {
			XLTJ[i]=(8019+i)+"";
		}
		MZTJ=new String[17];//帽子特技
		for (int i = 0; i < MZTJ.length; i++) {
			MZTJ[i]=(8018+i)+"";
		}
		XTJ=new String[16];//鞋特技
...
//      assetUpdate.setValueD("灵修值=-"+lxzv);
	...
	public void type10(LoginResult loginResult,ChannelHandlerContext ctx,SuitOperBean suitOperBean){
		...
		if (!Goodtype.Weapons(good.getType())) {
			SendMessage.sendMessageToSlef(ctx, Agreement.getAgreement().PromptAgreement("该类型装备禁止开光，炼器！！！#24"));
			return;
		}
		//防止外挂刷其他类型装备炼器
	...
	public void type11(LoginResult loginResult,ChannelHandlerContext ctx,SuitOperBean suitOperBean){
		...
		if (suitOperBean.getType()!=13) {
			...
			if (jade!=null) {
				...
				if (size>4) {return;}
				if (size!=0) {
					if (size==1) {money2=new BigDecimal(200);
					} else if (size==2) {
						money2=new BigDecimal(500);
					} else if (size==3) {
						money2=new BigDecimal(1000);
					} else {
						money2=new BigDecimal(2000);
					}
			}
			}
			if (money2!=null) {
				if (loginResult.getCodecard().compareTo(money2)<0) {return;}
			...
			MonitorUtil.getMoney().useD(money.longValue());
			String extra=getExtra(goods.get(0).getValue(), Extras[1]);
//			if (extra==null) {extra="炼器属性&0";}      //原版
			//防止外挂不开光炼器
			if (extra==null) {
				SendMessage.sendMessageToSlef(ctx, Agreement.getAgreement().PromptAgreement("武器都没开光就炼器，你是煞笔吗？#24"));
				return;
			}
			//防止外挂不开光炼器
			String[] v=extra.split("&");
			...
			if (kglvl>5) {return;}
			size++;
			extra=RefinersV(v,kglvl,lock,size, ((Goodstable)goods.get(0)).getType());
	...
	public void type16(LoginResult loginResult,ChannelHandlerContext ctx,SuitOperBean suitOperBean){
		...
		if (l==2&&ylvl<Integer.parseInt(configure.getSbjldj())) {
			SendMessage.sendMessageToSlef(ctx,Agreement.getAgreement().PromptAgreement(configure.getSbjldj()+"级及以上神兵才可以精炼"));
//			SendMessage.sendMessageToSlef(ctx, Agreement.getAgreement().PromptAgreement("7级及以上神兵才可以精炼"));
			return;
		}
		int v=1;
...
//		case 5:v=l==2?5:80;break;
		...
		case 1:v = l == 2 ? 38000 : 38000; break;
		    case 2: 
		      v = l == 2 ? 30000 : 30000; break;
		    case 3: 
		      v = l == 2 ? 500 : 500; break;
		    case 4: 
		      v = l == 2 ? 50 : 50; break;
...
//    	等级=4|敏捷要求=60|敏捷=6|加气血=1200|力量=6|灵性=6|培养=0/30|标签=4|值=1
	...
	/**生成炼器属性*/
	public static String RefinersV(String[] vs, int lvl, int lock, int min, long type){
		int max = Config.getInt("pt_lianqi");
		if (Goodtype.GodEquipment_xian(type)) {
			max = Config.getInt("xq_lianqi");
		} else if (Goodtype.GodEquipment_God(type)) {
			max = Config.getInt("sb_lianqi");
		}

		int sum = RefinersS(max);
		if (min > sum) {
			sum = min;
		}
...
//		}
	...
	public static String lh(int lvl,Alchemy alchemy,double xs){
		...
		if (key.equals("力量")||key.equals("根骨")||key.equals("灵性")||key.equals("敏捷")||
				key.equals("附加攻击")||key.equals("加速度")||key.equals("附加速度")||key.equals("附加气血")||key.equals("加气血")||key.equals("加魔法")||key.equals("附加魔法")||
	...
	public void type120(  LoginResult loginResult,ChannelHandlerContext ctx, SuitOperBean suitOperBean){
		...
		for (String vv : value) {
			if (vv.startsWith("培养")) {
				String mes1= ReadACardUtil.jiezhi.get(Battlefield.random.nextInt(ReadACardUtil.jiezhi.size()));
				String mix= ReadACardUtil.shuxing1.get(mes1);
				String mes2= ReadACardUtil.jiezhi.get(Battlefield.random.nextInt(ReadACardUtil.jiezhi.size()));
			...
			if (vv.startsWith("培养")) {
				...
				if (zong<5){
					...
					if (Battlefield.random.nextInt(100)<40){
						shuxing1+=Battlefield.random.nextInt(5)-2;
						shuxing2+=Battlefield.random.nextInt(3)-2;
					}
