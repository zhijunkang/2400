...
public class WingCompose {
	...
	public static void type31(LoginResult loginResult,ChannelHandlerContext ctx,SuitOperBean suitOperBean){
        ...
        if (jade==null) {return;}
        BigDecimal money=new BigDecimal(10000000L*jade.getPartId());
        if (loginResult.getGold().compareTo(money)<0) {return;}//金钱不足
        Goodstable pGood=AllServiceUtil.getGoodsTableService().getGoodsByRgID(new BigDecimal(jade.getSuitid() < 0 ? 4294967296l + jade.getSuitid()  : jade.getSuitid()));
	...
	public static void type35(LoginResult loginResult,ChannelHandlerContext ctx,SuitOperBean suitOperBean){
		...
		if (jade!=null) {
    		...
    		if (size!=0) {
    			if (size==1) {money2=new BigDecimal(40);}
    			else if (size==2) {money2=new BigDecimal(60);}
    			else {money2=new BigDecimal(100);}
    		}
    	}
        if (loginResult.getGold().compareTo(money)<0) {return;}//金钱不足
