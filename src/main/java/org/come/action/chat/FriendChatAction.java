...
//		LoginResult roleInfo=AllServiceUtil.getRoleTableService().selectRoleID(role_id);
		LoginResult roleInfo = GameServer.getAllLoginRole().get(ctx);
		int times = roleInfo.getGameTimeRemaining();
//		if (times<601) {
//			SendMessage.sendMessageToSlef(ctx,Agreement.getAgreement().PromptAgreement("#R注意：#Y游戏点数不足#G600#Y无法发言！请及时补充！"));		
//			return;
//		}
		...
		if(chatBean.getFriendName().equals("大话精灵")) {
			...
			chatBean1.setFriendName(chatBean.getRolename());//接收人
			chatBean1.setRolename(chatBean.getFriendName());//发送人
			chatBean1.setMessage(SpiritChat.getAnswering(chatBean.getMessage()));
			chatBean1.setTime(new Date().getTime());
			// 获得聊天内容
			String msg = Agreement.getAgreement().friendchatAgreement(GsonUtil.getGsonUtil().getgson().toJson(chatBean1));
			// 根据好友名字获取对方输出流，发送消息
			if(( GameServer.getRoleNameMap().get(chatBean1.getFriendName()) != null )&&(chatBean1.getFriendName()!=null)){
				// 获得该用户的输出流发送聊天内容
				SendMessage.sendMessageByRoleName(chatBean1.getFriendName(), msg);
			}
		}else {
			// 获得聊天内容
