...
//				RolePool.getRoleData(loginResult.getRole_id()).setMid(null);
		...
		}else if (message.split("&")[0].equals("OPEN")){
			...
			if (message.split("&").length==2){
				...
				for (RoleSummoning pet1:pets){
					if (pet1.getSid().toString().equals(message.split("&")[1])){
						pet=pet1;
			...
			SendMessage.sendMessageToSlef(ctx,Agreement.getAgreement().rolechangeAgreement("XY-"+ xy));
		}else if (message.split("&")[0].equals("OPEN1")){
			RoleSummoning pet=null;
			if (message.split("&").length==2){
				List<RoleSummoning> pets=
						AllServiceUtil.getRoleSummoningService().selectRoleSummoningsByRoleID(loginResult.getRole_id());
	...
	private void XY(LoginResult loginResult,String message){
		...
		if (message.split("#")[1].split("&")[0].equals("重置"))
		{
			if (message.split("&").length==2){
				...
				for (RoleSummoning pet1:pets){
					if (pet1.getSid().toString().equals(message.split("#")[1].split("&")[1])){
						pet=pet1;
						//pet= AllServiceUtil.getRoleSummoningService().selectRoleSummoningsByRgID(pet1.getSid());
					}
				}
			...
			String[] css=new String[]{"728","729"};
			//成圣初值增加
			pet.setXy(null);
			pet.setGrade(0);
			pet.setBone(0);pet.setSpir(0);
			pet.setPower(0);pet.setSpeed(0);
			pet.setCalm(0);pet.setExp(new BigDecimal(0));
			pet.setTurnRount(BattleMixDeal.petTurnRount(0));
			pet.setFriendliness(0L);pet.setFaithful(100);
			...
			pet.setSummoningskin(css[Battlefield.random.nextInt(css.length)]);
	...
	public void XP(LoginResult loginResult,String message){
		...
		if ("3".equals(V)) {
			if (xingpans == null) {
				...
				Goodstable goodstable=good.get(0);
				if (goodstable!=null) {
					System.out.println(good.get(0).getUsetime());
					if (shuliang>good.get(0).getUsetime()) {
						SendMessage.sendMessageByRoleName(loginResult.getRolename(),Agreement.getAgreement().PromptAgreement("道具不足"));
						return;
					}
