...
public class QianDaoAction implements IAction {
    ...
    private void giveGoodByType(ChannelHandlerContext ctx, LoginResult loginResult, int lqGoodType) {
    	...
    	Map<Integer, List<QianDao>> map = GameServer.getQianDaoMap().values().stream().collect(Collectors.groupingBy(QianDao::getDay));
    	
    	for (Entry<Integer, List<QianDao>> entry : map.entrySet()) {
